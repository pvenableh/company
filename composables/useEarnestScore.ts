...
function calculateEarnestScore(data: EarnestInputData): { score: number; dimensions: DimensionScores } {
	// Follow-Through (30 max)
	const completionRate = data.totalStarted > 0 ? (data.completed / data.totalStarted) * 20 : 10;
	const overduePenalty = Math.min(data.overdueItems * 2, 10);
	const followThrough = Math.max(0, Math.min(30, completionRate - overduePenalty + 10));

	// Consistency (25 max)
	const streakPoints = Math.min(data.streak * 1.5, 15);
	const weekBreadth = (Math.min(data.daysActiveThisWeek, 5) / 5) * 10;
	const consistency = Math.min(25, streakPoints + weekBreadth);

	// Responsiveness (20 max)
	const messageResponse = data.unreadMessages < 5 ? 10 : Math.max(0, 10 - (data.unreadMessages - 5));
	const followUpTimeliness = data.totalFollowups > 0 ? (data.timelyFollowups / data.totalFollowups) * 10 : 5;
	const responsiveness = Math.min(20, messageResponse + followUpTimeliness);

	// Proactivity (15 max)
	const scheduling = data.scheduledPosts > 0 ? 5 : 0;
	const earlyCompletions = Math.min(data.earlyCompletions * 2, 5);
	const projectPlanning = data.activeProjects > 0 ? (data.projectsWithDeadlines / data.activeProjects) * 5 : 5;
	const proactivity = Math.min(15, scheduling + earlyCompletions + projectPlanning);
	...
	const score = Math.round(followThrough + consistency + responsiveness + proactivity + depth);
...
export const useEarnestScore = () => {
	...
	const state = ref<EarnestState>({
		...
		streak: 0,
		bestStreak: 0,
		dimensions: { followThrough: 0, consistency: 0, responsiveness: 0, proactivity: 0, depth: 0 },
		...
		try {
			...
			if (records && records.length > 0) {
				...
				state.value = {
					...
					dimensions: r.dimension_scores || { followThrough: 0, consistency: 0, responsiveness: 0, proactivity: 0, depth: 0 },
					badges: BADGES.map((b) => ({
						id: b.id,
						name: b.name,
						icon: b.icon,

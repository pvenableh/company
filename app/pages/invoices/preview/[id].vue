...
const invoice = await invoiceItems.get(params.id, {
	fields: [
		'id,status,due_date,invoice_date,invoice_code,note,memo,total_amount,billing_email,billing_name,billing_address,emails,bill_to.id,bill_to.name,bill_to.email,bill_to.stripe_customer_id,bill_to.address,bill_to.phone,bill_to.website,bill_to.logo,bill_to.plan,bill_to.whitelabel,bill_to.document_theme,bill_to.document_accent,client.id,client.name,client.billing_name,client.billing_email,client.billing_address,client.billing_contacts,line_items.id,line_items.description,line_items.quantity,line_items.rate,line_items.amount,line_items.product.name',
	],
});
</script>
<template>

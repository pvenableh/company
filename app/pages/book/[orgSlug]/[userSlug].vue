...
<!--
  Org-scoped public booking page.

  URL shape: /book/<organizations.slug>/<scheduler_settings.booking_page_slug | user.id>
  The org slug provides the multitenant namespace so two users named "peter"
  in different orgs don't collide. The user slug is per-user-customizable but
  defaults to the user UUID; it's resolved against the org via the public-booking
  endpoint, which 404s if the user doesn't actually belong to that org.
-->
<script setup>
...
const fetchBookingData = async () => {
	...
	try {
		...
		hostOrg.value = response.organization;
...
const selectTime = (slot) => { selectedTime.value = slot; step.value = 2; };
const selectMeetingType = (type) => { bookingForm.meetingType = type.value; selectedDuration.value = type.duration; };
...
<template>
	<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
			...
			<div class="max-w-3xl mx-auto px-4 py-6">
				<div v-if="hostUser" class="flex items-center gap-4">
					...
					<div>
						...
						<p v-if="hostOrg?.name" class="text-xs text-gray-400 mt-0.5">{{ hostOrg.name }}</p>
					</div>
				</div>
			</div>
		</div>
			...
			<div v-else-if="step === 3" class="text-center py-8">
				...
				<div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-6 text-left max-w-md mx-auto">
					...
					<div class="space-y-2 text-sm">
						<div class="flex justify-between"><span class="text-gray-500">Type:</span><span>{{ selectedMeetingType?.label }}</span></div>
						<div class="flex justify-between"><span class="text-gray-500">Date:</span><span>{{ selectedDate?.formatted }}</span></div>
						<div class="flex justify-between"><span class="text-gray-500">Time:</span><span>{{ selectedTime?.formatted }}</span></div>
						<div class="flex justify-between"><span class="text-gray-500">Duration:</span><span>{{ selectedDuration }} minutes</span></div>
						<div class="flex justify-between"><span class="text-gray-500">With:</span><span>{{ hostUser?.first_name }} {{ hostUser?.last_name }}</span></div>
					</div>
				</div>
		...
		<div class="text-center py-8 text-sm text-gray-400">
			<template v-if="hostOrg && hostOrg.whitelabel">{{ hostOrg.name }}</template>
			<template v-else>Powered by <a href="https://earnest.guru" class="hover:text-gray-600">Earnest</a></template>
		</div>
	</div>
</template>

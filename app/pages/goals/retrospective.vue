...
useHead({ title: 'Retrospective | Earnest' });
...
const window = computed(() => periodWindow(timeframe.value));

const inWindowGoals = computed(() => {
	const startTs = window.value.start.getTime();
	const endTs = window.value.end.getTime();
	...
	try {
		...
		const res = await $fetch<Retrospective>('/api/ai/goal-retrospective', {
			...
			body: {
				...
				periodStart: window.value.start.toISOString().slice(0, 10),
				periodEnd: window.value.end.toISOString().slice(0, 10),
				goals: goalsPayload,
...
<template>
	<LayoutPageContainer>
		<div class="flex items-center justify-between mb-4">
			<div>
				<NuxtLink to="/goals" class="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1 mb-1">
					<UIcon name="i-heroicons-chevron-left" class="w-3.5 h-3.5" />
					Back to goals
				</NuxtLink>
				<h1 class="text-xl font-semibold">Retrospective</h1>
			...
			<div class="flex items-center gap-2">
				<div class="inline-flex items-center gap-1 rounded-full border border-border bg-card p-0.5">
					<button
						...
						:class="timeframe === 'month' ? 'bg-foreground text-background' : 'text-muted-foreground hover:text-foreground'"
					...
					<button
						type="button"
						class="rounded-full px-3 py-1 text-xs font-medium transition-colors"
						:class="timeframe === 'quarter' ? 'bg-foreground text-background' : 'text-muted-foreground hover:text-foreground'"
		...
		<div class="ios-card p-5 mb-4">
			<div class="flex items-center justify-between mb-3">
				<div>
					<h3 class="text-sm font-semibold">{{ window.label }}</h3>

<!--
	RolesBody — shared inner body for the Roles & Permissions admin surface.

	Rendered by both the legacy `/organization/roles` route (now a redirect)
	and the apps-layout slide-over `roles` panel. The `compact` prop drops
	the page-level heading so the slide-over shell can supply its own.
-->
<template>
	<div>
		...
		<div v-else>
			<!-- Role Cards -->
			<div :class="roleCardGridClass" class="gap-3 mb-8">
				<button
					v-for="role in roles"
					:key="role.id"
...
// Five-up grid in a full page, three-up in the narrower slide-over.
const roleCardGridClass = computed(() => props.compact

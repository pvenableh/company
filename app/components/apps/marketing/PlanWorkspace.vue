<!--
  PlanWorkspace — Camila's content-plan workspace body.

  Consumed by both the `/social/plans/[id]` full-page route AND the
  `social-plan` slide-over panel. Pass `:plan-id`; the component fetches
  itself. Emits `@loaded` with the plan record (panel uses it to populate
  the slide-over shell title).

  `:compact` hides chrome that the slide-over shell already provides
  (the hero back-button) and routes "back" → `@back` instead of a
  router push so the panel can `pop()`.
-->
...
const emit = defineEmits<{
  (e: 'loaded', plan: ContentPlanRecord): void;
  (e: 'back'): void;
}>();
...
async function fetchPlan() {
  ...
  try {
    const r = await $fetch<{ data: { plan: ContentPlanRecord; posts: SocialPost[] } }>(
      `/api/social/plans/${props.planId}`,
    );
    plan.value = r?.data?.plan ?? null;
    posts.value = r?.data?.posts ?? [];
    if (plan.value) emit('loaded', plan.value);
  } catch (err: any) {
...
// Re-fetch when the planId prop changes (panel reuse when navigating
// between plans without unmount).
watch(() => props.planId, () => {
  plan.value = null;
  posts.value = [];
  fetchPlan();
...
async function patchPlan(patch: Partial<ContentPlanRecord>) {
  ...
  pending.value = true;
  try {
    const r = await $fetch<{ data: ContentPlanRecord }>(`/api/social/plans/${props.planId}`, {
      method: 'PATCH',
      body: patch,
    });
...
async function addPost() {
  ...
  try {
    ...
    await $fetch<{ data: SocialPost }>('/api/social/posts', {
      method: 'POST',
      body: {
...
async function sendForReview() {
  ...
  try {
    const r = await $fetch<{ data: { plan: ContentPlanRecord; postsTransitioned: number; reviewUrl: string | null } }>(
      `/api/social/plans/${props.planId}/send-for-review`,
      { method: 'POST' },
    );
...
function openPortalView() {
  if (!plan.value?.approval_token) return;
  window.open(`/portal/plans/${plan.value.approval_token}`, '_blank', 'noopener,noreferrer');
}
...
// Hero back button. Page mode → push back to Studio; panel mode → emit
// so the parent can pop().
function onBack() {
  if (props.compact) {
    emit('back');
  } else {
    router.push('/apps/marketing?floor=studio');
  }
}
    ...
    <template v-else-if="plan">
      ...
      <section class="plan-hero">
        ...
        <div class="flex items-center gap-2 shrink-0">
          ...
          <UiActionButton
            v-if="plan.approval_token"
            icon="lucide:external-link"
            title="Open the client-facing portal view in a new tab"
            @click="openPortalView"
          >
            View as Client
          </UiActionButton>
          <UiActionButton
...
/* In a slide-over panel: drop the outer max-width + horizontal padding so
 * the workspace consumes the panel's content padding instead. */
.plan-shell--compact {
  @apply max-w-none mx-0 px-0 py-0 space-y-4;
}
...
/* In a slide-over panel the rail goes under the main column — at panel
 * width (max-w-2xl) the desktop two-column grid is awkward. */
.plan-shell--compact .plan-layout {
...
.plan-shell--compact .plan-rail__sticky {
  @apply lg:static lg:top-auto;
}

<!--
  Shared tab pill row for the client surface. Both the full-page detail and
  the slide-over render the same tabs with the same counts so the two
  surfaces stay aligned. Mirrors AppsWorkProjectTabsBar, including the app
  floor nav's sliding-thumb animation.
-->
...
const HAS_COUNT = (key: ClientTabKey) => !['overview', 'activity'].includes(key);

// ── Sliding thumb (mirrors AppFloorStrip / ProjectTabsBar) ─────────────
...
<template>
	...
	<div ref="scrollerEl" class="client-tabs relative flex flex-nowrap items-center gap-1 overflow-x-auto -mx-1 px-1 scrollbar-hide">
		...
		<button
			...
			:key="tab.key"
			:ref="setItemRef(idx)"
			class="client-tabs__item"
			:class="{ 'client-tabs__item--active': modelValue === tab.key }"

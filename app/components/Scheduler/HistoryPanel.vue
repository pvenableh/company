<script setup>
/**
 * SchedulerHistoryPanel — meeting recap archive, surfaced as slide-out content
 * from the Scheduler hub (replaces the standalone /meetings list page).
 *
 * Self-contained: fetches video meetings where the current user is host or
 * attendee, with search + recap filtering. Rows deep-link to the full
 * /meetings/[id] recap detail page. Works for admin (calendar hub) and
 * non-admin (booking) scheduler views alike.
 */
...
const fetchMeetings = async () => {
	...
	try {
		...
		console.error('[SchedulerHistoryPanel] fetch failed', err);
		meetings.value = [];
	}
	loading.value = false;
};
...
// `~/utils/palette-tokens` (auto-imported), in lockstep with meetings/[id].vue.
const toneClass = (tone) => softTone(tone);

defineExpose({ refresh: fetchMeetings });
</script>
	...
	<div class="p-4 sm:p-5">
		...
		<div v-else-if="filtered.length === 0" class="ios-card p-10 text-center">
			...
			<p class="text-sm text-muted-foreground mt-1.5 max-w-sm mx-auto">
				Start an instant meeting or schedule one from the calendar. Recaps appear here after the host enables transcription during the call.
			</p>
		</div>

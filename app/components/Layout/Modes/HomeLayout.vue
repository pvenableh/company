...
const domainMap: Record<string, { parent?: { label: string; to: string }; label: string }> = {
	'/projects': { parent: { label: 'Work', to: '/projects' }, label: 'Projects' },
	'/tickets': { parent: { label: 'Work', to: '/projects' }, label: 'Tickets' },
	'/tasks': { parent: { label: 'Work', to: '/projects' }, label: 'Tasks' },
	'/scheduler': { parent: { label: 'Work', to: '/projects' }, label: 'Scheduler' },
	'/files': { parent: { label: 'Work', to: '/projects' }, label: 'Files' },
	'/goals': { parent: { label: 'Work', to: '/projects' }, label: 'Goals' },
	'/contacts': { parent: { label: 'Pipeline', to: '/leads' }, label: 'Contacts' },
	'/clients': { parent: { label: 'Pipeline', to: '/leads' }, label: 'Clients' },
	'/leads': { parent: { label: 'Pipeline', to: '/leads' }, label: 'Leads' },
	'/proposals': { parent: { label: 'Pipeline', to: '/leads' }, label: 'Proposals' },
	'/contracts': { parent: { label: 'Pipeline', to: '/leads' }, label: 'Contracts' },
	'/channels': { label: 'Team' },
		...
		<header class="glass flex items-center justify-between border-b border-border/40 px-4 lg:px-6 h-12 shrink-0 z-40">
			...
			<div class="flex items-center gap-2 min-w-0 flex-1">
				<button
					v-if="!isHome"
					@click="goBack"
					class="flex items-center justify-center w-8 h-8 rounded-full hover:bg-muted/50 text-muted-foreground transition-colors shrink-0"
				>

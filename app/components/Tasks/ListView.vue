<template>
	<div class="w-full">
		...
		<!-- Task edit/detail is the shared `task` slide-over panel now (pushed
		     from a row click), not an inline bottom sheet. -->
...
const { flipFrom, captureFromEl } = useFlipFromRow();
const taskSlide = useAppSlideOver('task');

function openEdit(task: any, ev: MouseEvent) {
	// Capture the clicked row's rect so the task panel FLIPs it into its hero,
	// then push the shared `task` panel. Inside a workspace panel this stacks
	// on top with a back chevron; on a page it opens at depth 1. Replaces the
	// old inline edit bottom sheet (which duplicated the task workspace).
	const row = (ev.currentTarget as HTMLElement).closest('.task-row') as HTMLElement | null;
	captureFromEl(row);
	taskSlide.open(String(task.id), { flipFrom: flipFrom.value ?? undefined });
}

// The task panel edits via TaskWorkspace, which notifies the entity bus;
// refresh the list so status/field changes reflect here.

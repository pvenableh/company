		...
		try {
			...
			const [junctionOrgs, memberships, portalRows] = await Promise.all([
				orgItems.list({
					...
					fields: ['id', 'name', 'slug', 'logo', 'icon', 'plan', 'folder', 'active_addons', 'stripe_customer_id', 'stripe_subscription_id', 'default_hourly_rate', 'email', 'phone', 'address', 'archived_at', 'whitelabel', 'document_theme', 'document_accent', 'document_theme_config', 'brand_color', 'brand_direction', 'app_palette', 'goals', 'goals_enabled', 'weather_enabled', 'target_audience', 'location', 'website', 'notes', 'email_reply_to', 'mailing_address', 'email_bcc'],
				}),
			...
			const processed = data.map((org) => {
				...
				return {
					...
					// Opt-in: the header weather widget is OFF unless explicitly enabled.
					weather_enabled: org.weather_enabled === true,

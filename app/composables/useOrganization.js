		...
		try {
			...
			const processed = data.map((org) => {
				...
				return {
					...
					// Opt-out: Teams are ON unless explicitly disabled (so orgs already
					// using teams aren't disrupted); only an explicit `false` hides them.
					teams_enabled: org.teams_enabled !== false,

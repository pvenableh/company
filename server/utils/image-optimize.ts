 ...
 * Uses `sharp` (already a dependency) to shrink oversized photos.
 *
 * Output format:
...
export async function optimizeImageBuffer(
  ...
  try {
    ...
    if (format === 'webp') {
      ...
      out = await pipeline.png({ compressionLevel: 9, palette: true, quality: 82, effort: 7 }).toBuffer();
      outType = 'image/png';
    ...
    // Only take the optimized version if it genuinely saves space AND we didn't
    // just re-encode into the identical format at a larger size.

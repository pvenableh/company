...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const orgContext = {
      ...
      momentumSummary: cachedContext?.momentumSummary || undefined,
    };
...
- Only include essential details`;
    ...
    const toolNudge = useMutationTools
      ? `\n\nTOOLS ENABLED: You can mutate this entity directly. The user's currently focused record is ${entityType}="${entityId}" in organization="${organizationId}". When the user asks to reschedule, update, or add — call the matching tool (reschedule_project / update_field / add_task) with this exact id. When the user asks to draft/write up a proposal or contract, call generate_documents with an overview of the deliverables (it saves editable drafts for review — it does not send anything). When the user asks to email/follow up with/reach out to someone, call send_email to PROPOSE the email — this does NOT send; it queues the email for the user to approve in the AI Activity queue. After calling send_email, tell the user you've drafted and queued the email for their approval — never say it was sent. When the user asks about their availability or to find a time ("when am I free", "find a time"), call find_a_time (read-only) and offer the returned slots. When the user asks to book/schedule/set up a meeting or call with someone, call book_meeting to PROPOSE it (queued for approval; nothing is booked and no invite goes out until approved) — resolve the invitee from the focused contact or an explicit email. When the user asks to move/reschedule or cancel a specific meeting, call reschedule_meeting / cancel_meeting to PROPOSE it. For all of these, say you've *proposed* the action and it's awaiting approval — never say the meeting was booked, moved, or cancelled. Do NOT refuse or describe the change; execute it. If a tool returns success:false, surface the error verbatim instead of inventing a permission excuse.`
      : '';

 ...
 * GET /api/social/oauth/instagram/webhook
 *
 * Meta sends a verification request when you save the webhook URL in the app
 * dashboard. We must echo back `hub.challenge` if the verify token matches.
...
export default defineEventHandler((event) => {
  ...
  if (!expected) {
    throw createError({ statusCode: 500, statusMessage: 'INSTAGRAM_WEBHOOK_VERIFY_TOKEN not configured' })
  }

 ...
 * GET /api/social/oauth/threads/webhook
 *
 * Meta sends a verification request when you save the webhook URL in the app
 * dashboard (Threads product → Webhooks). We must echo back `hub.challenge`
...
export default defineEventHandler((event) => {
  ...
  const expected = process.env.THREADS_WEBHOOK_VERIFY_TOKEN

  if (!expected) {
    throw createError({ statusCode: 500, statusMessage: 'THREADS_WEBHOOK_VERIFY_TOKEN not configured' })
  }

...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const checkoutSession = await stripe.checkout.sessions.create({
      ...
      success_url: successUrl || `${getAppBaseUrl(event)}/apps/organization?floor=ai&tokens_purchased=true`,
      cancel_url: cancelUrl || `${getAppBaseUrl(event)}/apps/organization?floor=ai`,
    });

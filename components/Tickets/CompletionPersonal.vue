<template>
	<div class="w-full h-full">
		...
		<client-only>
			<Bar v-if="chartData.labels.length" :options="chartOptions" :data="chartData" :height="null" :width="null" />
		</client-only>
	</div>
</template>
...
const processedData = computed(() => {
	...
	return props.data.map((item) => ({
		name: item.name,
		completed: item.rate > 0 ? item.rate : null, // Use rate as completed, null if no rate
	}));
});

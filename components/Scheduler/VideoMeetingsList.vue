<!-- components/Scheduler/VideoMeetingsList.vue -->
<script setup>
...
const sendInvite = async () => {
	...
	try {
		if (selectedInviteTab.value === 0 && inviteForm.email) {
		...
		} else if (selectedInviteTab.value === 1 && inviteForm.phone) {
...
<template>
	<div>
		<div v-if="meetings.length === 0" class="text-center py-12">
			...
			<p class="text-gray-500 mb-4">Create a video meeting to get started</p>
		</div>
			...
			<UCard>
							...
							<div class="space-y-4 pt-4">
								...
								<template v-else>
									<UFormGroup label="Meeting Link">
										<div class="flex gap-2">
											<UInput :model-value="selectedMeeting.meeting_url" readonly class="flex-1" />
				...
				<template #footer>
					<div class="flex justify-end gap-3">
						...
						<UButton v-if="selectedInviteTab !== 2" color="primary" @click="sendInvite">
							Send {{ selectedInviteTab === 0 ? 'Email' : 'SMS' }}
						</UButton>
					</div>
				</template>
			</UCard>

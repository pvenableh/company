#!/usr/bin/env npx tsx
/**
 * Directus Organizations Collection — Permissions Setup Script
 *
 * Configures read/write permissions for the `organizations` collection
 * across non-admin roles. Fixes the 403 error when users try to update
 * organization details (goals, location, brand_direction, etc.).
 *
 * Permissions:
 *   - Client Manager: read, update, create (full org management)
 *   - User (Carddesk):  read only (view org details)
 *
 * Usage:
 *   pnpm tsx scripts/setup-organization-permissions.ts
 */
...
async function main() {
  console.log('=========================================')
  console.log('  Organizations — Permissions Setup')
  console.log('=========================================')
  ...
  for (const role of roles) {
    ...
    if (!policyId) {
      ...
      const policyName = `${role.name} - Organizations`

      const { data: newPolicy, error: policyError } = await directusRequest<{ id: string }>('/policies', 'POST', {
        name: policyName,
        icon: 'business',
        description: `Auto-generated permissions for organizations collection (${role.name})`,
        admin_access: false,
        app_access: true,
      })
  ...
  if (totalFailed === 0) {
    ...
    console.log('  organizations  - Client Managers can read, update, create')
    console.log('                 - Users can read org details')
  }
}

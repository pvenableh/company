#!/usr/bin/env npx tsx
/**
 * Adds `app_pref_studio_intro_dismissed_at` (timestamp) to directus_users.
 *
 * Drives the first-visit intro modal on /social/plans/[id] (the Content
 * Studio plan editor). Semantics mirror the CardDesk promo:
 *   - null           → show the intro
 *   - any timestamp  → already dismissed; never re-show automatically
 *
 * The intro is re-openable from an "i" badge in the plan editor header, so
 * we don't need snooze semantics here.
 *
 * Run:
 *   pnpm tsx scripts/setup-studio-intro-field.ts
...
async function main() {
  console.log('\n── Studio Intro Dismissal Field Setup ──\n')
  console.log(`Directus: ${DIRECTUS_URL}\n`)

  console.log('directus_users collection:')
  await createField('directus_users', 'app_pref_studio_intro_dismissed_at', {
    type: 'timestamp',
    meta: {
      interface: 'datetime',
      display: 'datetime',
      note: 'Content Studio first-visit intro dismissal. Null = show; any timestamp = already dismissed.',
      hidden: false,
      width: 'half',
    },
    schema: {

...
Earnest is built to feel like a native iOS app, not a web dashboard. Three primitives carry the entire app:

- **Native iOS motion.** One spring curve — `cubic-bezier(0.36, 0.66, 0.04, 1)` @ 400ms — drives every push, pop, slide, and dismiss. Detail surfaces open as slide-over panels that stack push/pop with a Framework7-style spring; the prior panel scales back and fades, the list stays visible behind, the URL travels with you. Create and edit flows use `AppBottomSheet` with a drag-to-dismiss handle and safe-area inset. Inline interactions add iOS-style segmented controls, a sliding floor pill on tab switches, haptic taps, `scale(0.96)` press feedback, route transitions, and an iOS-style activity spinner.
- **Liquid glass system.** Three system tiers — `.glass-thin`, `.glass`, `.glass-ultra` — are applied universally to chrome (toolbars, sheets, dropdowns, tooltips, dialogs, toasts). On top of those, the hue-driven `.glass-surface` primitive picks up the active app accent so glass in Work feels different from glass in Marketing while staying coherent. Dialog overlays get an accent-tinted radial-gradient blur (`.dialog-overlay-glass`) so the underlying app reads as "behind glass" instead of dimmed out. Inputs and textareas get an iOS-style accent focus ring. Everything falls back to a flat tinted card under `prefers-reduced-transparency` and to a solid background where `backdrop-filter` is unsupported.
- **River timeline.** A reusable time-on-a-river visualization (`RiverChart.vue`) — a horizontally-scrolling day strip with 24-hour beds, channel-tinted breathing leaves per item, a temperature curve that exposes rhythm gaps, and a glowing "now" indicator. It backs the Social Content Studio (drag-to-reschedule via `SocialRiverSurface.vue`), payments, tasks workload, and campaign scheduling. Read-only surfaces use it for a glance-level view of past + future; the Studio uses it as the primary canvas.

These primitives are policy, not opt-in styling — every new surface adopts them so the app keeps moving and feeling like one thing. See `app/assets/css/theme.css` for the glass tiers, `app/components/river/RiverChart.vue` for the river primitive, and `app/components/apps/AppSlideOverStack.vue` + `AppBottomSheet.vue` for the iOS navigation building blocks.
...
- **Social Media Management** — Compose, schedule, and publish to Instagram, TikTok, LinkedIn, Facebook Pages, and Threads; AI-powered content wizard generates platform-optimized posts with tailored copy, hashtags, and image suggestions; content calendar, engagement analytics, multi-client management, and OAuth account connections. The **Social Content Studio** lays posts onto the **river timeline** — a day strip with hour beds, channel-tinted leaves, and a temperature curve — where you can drag a leaf to reschedule, lift one to compose, and read your rhythm at a glance
...
- **Four Navigation Modes** — Choose your preferred workspace layout: **Apps** (the new default — a thin app-rail with circular gradient app chips per "app" — Work, Pipeline, Financials, Engage, Marketing, Clients, Settings — over a palette-tinted glass plinth and a contextual second column for sub-nav. Detail surfaces open as full-height slide-overs that stack push/pop with a Framework7-style spring; the list stays visible behind, the prior panel scales back and fades, and the URL travels with you. Rail position (left/right/top/bottom/floating) and the palette-tint toggle are user-configurable.), **Spaces** (classic macOS-style sidebar with collapsible Work / Pipeline / Financials / Engage / Team sections and an in-page pill nav that mirrors the active section), **Tabs** (iPadOS-style 5-tab top bar: Work, Pipeline, Financials, Engage, Chat), or **Home** (Apple TV-style card grid with breadcrumb drill-down). Mode persists per user via localStorage. (The earlier "Hats" role-preset picker has been retired in favor of the Apps Layout's per-app surfaces.)

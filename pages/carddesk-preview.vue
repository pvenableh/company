...
// ─── Types ────────────────────────────────────────────────────────────────────
interface Activity { id:string;type:string;label:string;date:string;note:string;isResponse:boolean;responseNote?:string }
interface Contact  { id:string;name:string;title?:string;company?:string;email?:string;phone?:string;industry?:string;metAt?:string;rating:string;hibernated:boolean;hibernatedAt?:string;activities:Activity[];createdAt:string;notes?:string }
type Screen = 'vibe'|'session'|'cold'|'home'|'contacts'|'detail'|'add'

// ─── Constants ────────────────────────────────────────────────────────────────
const RATINGS       = [{key:'hot',label:'Hot',emoji:'🔥',color:'#ff6b35'},{key:'warm',label:'Warm',emoji:'👍',color:'#ffe033'},{key:'nurture',label:'Nurture',emoji:'🌱',color:'#00c268'},{key:'cold',label:'Cold',emoji:'❄️',color:'#4da6ff'}]
const ACT_TYPES     = [{key:'email',label:'Email',icon:'📧'},{key:'text',label:'Text',icon:'📱'},{key:'call',label:'Call',icon:'📞'},{key:'meeting',label:'Meeting',icon:'🤝'},{key:'linkedin',label:'LinkedIn',icon:'🔗'},{key:'other',label:'Other',icon:'💬'}]
const INDUSTRIES    = ['Technology','Finance','Healthcare','Real Estate','Legal','Marketing','Venture Capital','Other']
const EMOJIS        = ['🐯','🦁','🦊','🐺','🦋','🐬','🦉','🦝','🐠','🦌','🦅','🌊']
const LEVELS        = [{level:1,title:'Rookie',xp:0},{level:2,title:'Hustler',xp:200},{level:3,title:'Connector',xp:500},{level:4,title:'Player',xp:1000},{level:5,title:'Rainmaker',xp:2000},{level:6,title:'Closer',xp:5000},{level:7,title:'Networker',xp:10000},{level:8,title:'Dealmaker',xp:20000},{level:9,title:'Legend',xp:50000}]
const BADGES        = [{key:'card_shark',emoji:'🃏',label:'Card Shark',desc:'Scan 5 cards',check:(s:any)=>s.totalScans>=5},{key:'hot_streak',emoji:'🔥',label:'Hot Streak',desc:'7-day streak',check:(s:any)=>s.streak>=7},{key:'speed_dialer',emoji:'⚡',label:'Speed Dialer',desc:'Follow up within 24h',check:(s:any)=>s.fastFollowups>=1},{key:'networker',emoji:'🌐',label:'Networker',desc:'Add 10 contacts',check:(s:any)=>s.totalContacts>=10},{key:'dealmaker',emoji:'💎',label:'Dealmaker',desc:'Response from Hot lead',check:(s:any)=>s.hotResponses>=1},{key:'connector',emoji:'🌉',label:'Connector',desc:'Make 3 intros',check:(s:any)=>s.intros>=3},{key:'legend',emoji:'👑',label:'Legend',desc:'Reach Level 9',check:(s:any)=>s.level>=9}]
const MISSIONS      = [{key:'scan',icon:'📷',label:'Scan a Business Card',hype:'Done. Like a machine.',xp:50},{key:'followup',icon:'✉️',label:'Log a Follow-up',hype:"They'll remember you.",xp:25},{key:'hot',icon:'🔥',label:'Follow Up a Hot Lead',hype:"Don't leave them hanging.",xp:50},{key:'response',icon:'✅',label:'Log a Response',hype:'They came back. Of course they did.',xp:100}]
const TOUGH_CARDS   = [{q:'"Your hot leads are going cold right now."',b:'These conversations could change things — sitting there, waiting. You know what to do. <em>One text.</em> That\'s your entire task list. Not five. <strong>One.</strong>'},{q:'"The follow-up window doesn\'t stay open forever."',b:"But it's still open <em>right now.</em> You'll feel better the second you hit send. The hardest part is starting — so start. One line. Just real."},{q:'"Your streak is worth protecting tonight."',b:"You've been showing up. Don't let tonight break it. One touchpoint — doesn't have to be clever. Just <em>real.</em>"}]
const HYPE_CARDS    = [{q:'"Your response rate is well above average."',b:"People don't reply out of politeness — they reply because <strong>you're worth their time.</strong> You always have been. Go remind someone you exist."},{q:'"You are in the top tier of networkers."',b:"You show up, follow through, make people feel seen. That's a <em>rare</em> skill. Most people don't have it. <strong>You do. Own it.</strong>"},{q:'"Every great connection started with one message."',b:"Not a pitch deck. Not a post. <em>One message.</em> That's the whole playbook — and you're brilliant at it. <strong>Do it again.</strong>"}]
const COLD_WARMERS  = ["It's a bummer, but maybe they just need time to thaw out. That doesn't mean the connection isn't real. You met for a reason.","Sometimes the slow burns lead to the best leads — don't give up. Your patience is actually a competitive advantage.","Maybe they're just busy — remind them how you can enhance their world. A genuine 'thinking of you' goes further than you think."]
const VIBE_MOODS    = [{e:'😶‍🌫️',title:'Feeling antisocial lately?',color:'blue',body:"We get it. One message. That's it. You don't have to be \"on\" — just human. That's always enough."},{e:'🔥',title:'You are absolutely killing it.',color:'green',body:"Contacts growing, streak alive. People in your industry are noticing. You're not just networking — you're building a career asset."},{e:'😴',title:'A little tired today?',color:'blue',body:'Totally valid. A 2-second LinkedIn reaction keeps the connection warm and saves your streak. Minimum effort, maximum effect.'},{e:'🏆',title:'You are brilliant at this.',color:'green',body:"You show up, follow through, make people feel seen. That's not the app — that's you being exceptional."},{e:'🛋️',title:'Introverted this week?',color:'purple',body:"Pick your 3 hottest leads. One message each. Done in 5 minutes. That's your whole to-do list."}]
const XP_ACTIONS:Record<string,{xp:number,icon:string,msg:string}> = {scan_card:{xp:50,icon:'📷',msg:'Card scanned!'},log_followup:{xp:25,icon:'✉️',msg:"They'll remember you."},got_response:{xp:100,icon:'🎉',msg:'They came back. Of course they did.'},hot_followup:{xp:50,icon:'⚡',msg:"Don't leave them hanging."},complete_mission:{xp:50,icon:'🎯',msg:'Mission complete.'},unlock_badge:{xp:75,icon:'🏅',msg:'Badge unlocked!'},save_contact:{xp:25,icon:'💾',msg:"They're in your network."}}

// ─── State ────────────────────────────────────────────────────────────────────
const screen       = ref<Screen>('vibe')
const prevScreen   = ref<Screen>('contacts')
const contacts     = ref<Contact[]>([])
const xp           = ref({totalXP:0,level:1,streak:0,lastActivityDate:'',totalScans:0,totalContacts:0,fastFollowups:0,hotResponses:0,intros:0,unlockedBadges:[] as string[],completedMissions:[] as string[],missionsDate:''})
...
const cSearch      = ref(''); const cFilter = ref(''); const cSort = ref('recent')
const actType      = ref('email'); const actNote = ref(''); const actDate = ref(todayStr())
const addForm      = ref({firstName:'',lastName:'',title:'',company:'',email:'',phone:'',industry:'',metAt:'',rating:'',notes:''})

// ─── Helpers ──────────────────────────────────────────────────────────────────
function uid() { return Date.now().toString(36)+Math.random().toString(36).slice(2) }
function todayStr() { return new Date().toISOString().slice(0,10) }
function formatDate(d?:string) { if(!d) return ''; return new Date(d+'T00:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}) }
function fmtShort(d?:string)   { if(!d) return ''; return new Date(d+'T00:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'}) }
function daysSince(d?:string)  { if(!d) return null; return Math.floor((Date.now()-new Date(d).getTime())/86400000) }
function cEmoji(c:Contact)     { return EMOJIS[Math.abs((c.id.charCodeAt(0)||0)+(c.id.charCodeAt(1)||0))%EMOJIS.length] }
...
function lastAct(c:Contact)    { if(!c.activities?.length) return null; return [...c.activities].sort((a,b)=>new Date(b.date).getTime()-new Date(a.date).getTime())[0] }
...
function fuInfo(c:Contact) {
  ...
  return {
    overdue:{cls:'ov',ico:'⚡',title:`${d} days overdue — don't let this one slip`,sub:`Last: ${la?.label||'Contact'} on ${formatDate(la?.date)} · ${la?.isResponse?'✓ Replied':'No reply yet'}`},
    due:    {cls:'du',ico:'⏰',title:`${d} days since last contact — right on the line`,sub:'A quick touch now resets the clock.'},
    ok:     {cls:'ok',ico:'✓', title:"You're on top of this one",sub:`Last: ${la?.label||'Contact'} on ${formatDate(la?.date)} · ${la?.isResponse?'✓ Replied':'Awaiting reply'}`},
    new:    {cls:'nw',ico:'👋',title:'No activity logged yet',sub:'Use the form below to log your first touchpoint.'},
  }[s]
}
function coldWarmer(c:Contact) { const d=daysSinceLast(c)||0; if(d>=30) return COLD_WARMERS[1]; return COLD_WARMERS[Math.abs(c.id.charCodeAt(0))%3] }

// ─── Persistence ──────────────────────────────────────────────────────────────
function save() { if(typeof window==='undefined') return; localStorage.setItem('cd2_c',JSON.stringify(contacts.value)); localStorage.setItem('cd2_x',JSON.stringify(xp.value)) }
function load() {
  ...
  const today=todayStr(); if(xp.value.missionsDate!==today){xp.value.completedMissions=[];xp.value.missionsDate=today;save()}
}

// ─── XP Engine ────────────────────────────────────────────────────────────────
function showToast(xpStr:string,icon:string,msg:string) { toast.value={xp:xpStr,icon,msg}; clearTimeout(toastTimer.value); toastTimer.value=setTimeout(()=>toast.value=null,2400) }
function earnXP(action:string,extra:Record<string,any>={}) {
  const a=XP_ACTIONS[action]; if(!a) return
  xp.value.totalXP+=a.xp; Object.assign(xp.value,extra)
  for(let i=LEVELS.length-1;i>=0;i--){ if(xp.value.totalXP>=LEVELS[i].xp){xp.value.level=LEVELS[i].level;break} }
  const today=todayStr(),yesterday=new Date(Date.now()-86400000).toISOString().slice(0,10)
  if(xp.value.lastActivityDate!==today){ xp.value.streak=(xp.value.lastActivityDate===yesterday)?xp.value.streak+1:1; xp.value.lastActivityDate=today }
  BADGES.forEach(b=>{ if(!xp.value.unlockedBadges.includes(b.key)&&b.check(xp.value)){xp.value.unlockedBadges.push(b.key);xp.value.totalXP+=75;showToast('+75 XP',b.emoji,`${b.label} unlocked!`)} })
  save(); showToast(`+${a.xp} XP`,a.icon,a.msg)
}
...
const xpPct      = computed(()=>{ if(!nextLvl.value) return 100; const c=curLvl.value.xp,n=nextLvl.value.xp; return Math.min(100,Math.round(((xp.value.totalXP-c)/(n-c))*100)) })
const xpToNext   = computed(()=>nextLvl.value?nextLvl.value.xp-xp.value.totalXP:0)
const sDots      = computed(()=>{ const d=[]; for(let i=0;i<7;i++){const a=6-i;if(a===0)d.push(xp.value.streak>0?'today':'empty');else d.push(xp.value.streak>a?'done':'empty')} return d })
...
const sortedActs = computed(()=>{ if(!selContact.value?.activities?.length) return []; return [...selContact.value.activities].sort((a,b)=>new Date(b.date).getTime()-new Date(a.date).getTime()) })
...
const filteredCs = computed(()=>{
  ...
  let list=contacts.value.filter(c=>!c.hibernated&&(c.name?.toLowerCase().includes(q)||c.company?.toLowerCase().includes(q))&&(cFilter.value===''||c.rating===cFilter.value))
...
// ─── Navigation ───────────────────────────────────────────────────────────────
function nav(s:Screen) { prevScreen.value=screen.value; screen.value=s }
function goDetail(id:string) { selectedId.value=id; nav('detail') }

// ─── Actions ──────────────────────────────────────────────────────────────────
function toggleCold(id:string) { openCold.value=openCold.value.includes(id)?openCold.value.filter(x=>x!==id):[...openCold.value,id] }
function hibernate(id:string) { contacts.value=contacts.value.map(c=>c.id===id?{...c,hibernated:true,hibernatedAt:todayStr()}:c); openCold.value=openCold.value.filter(x=>x!==id&&x!==id+'_h'); save(); showToast('','😴','On ice — not gone.') }
function wake(id:string) { contacts.value=contacts.value.map(c=>c.id===id?{...c,hibernated:false,hibernatedAt:undefined}:c); save(); earnXP('log_followup') }
function logAct(isResp:boolean) {
  if(!selContact.value) return
  const act:Activity={id:uid(),type:actType.value,label:getAct(actType.value).label,date:actDate.value||todayStr(),note:actNote.value,isResponse:isResp}
  contacts.value=contacts.value.map(c=>c.id===selectedId.value?{...c,activities:[act,...(c.activities||[])]}:c); save()
  actNote.value=''; actDate.value=todayStr()
  if(isResp){earnXP('got_response',selContact.value?.rating==='hot'?{hotResponses:(xp.value.hotResponses||0)+1}:{}); fireConfetti()}
  else selContact.value?.rating==='hot'?earnXP('hot_followup'):earnXP('log_followup')
}
function toggleResp(actId:string) {
  if(!selContact.value) return
  contacts.value=contacts.value.map(c=>{ if(c.id!==selectedId.value) return c; return {...c,activities:c.activities.map(a=>a.id===actId?{...a,isResponse:true,responseNote:'Responded'}:a)} })
  save(); earnXP('got_response'); fireConfetti()
}
function saveContact() {
  if(!addName.value) return
  const id=uid(); const c:Contact={id,name:addName.value,title:addForm.value.title,company:addForm.value.company,email:addForm.value.email,phone:addForm.value.phone,industry:addForm.value.industry,metAt:addForm.value.metAt,notes:addForm.value.notes,rating:addForm.value.rating,hibernated:false,activities:[],createdAt:todayStr()}
  contacts.value=[c,...contacts.value]; save(); earnXP('save_contact',{totalContacts:contacts.value.length})
  addForm.value={firstName:'',lastName:'',title:'',company:'',email:'',phone:'',industry:'',metAt:'',rating:'',notes:''}; nav('contacts')
}
function doMission(key:string) { if(xp.value.completedMissions.includes(key)) return; xp.value.completedMissions.push(key); earnXP('complete_mission') }
function startEdit() { if(!selContact.value) return; editForm.value={...selContact.value}; editing.value=true }
function saveEdit() { contacts.value=contacts.value.map(c=>c.id===selectedId.value?{...c,...editForm.value}:c); save(); editing.value=false }
function fireConfetti() { confettiLib({particleCount:60,spread:70,origin:{y:0.6},colors:['#00ff87','#ffd700','#ff6b35','#4da6ff','#b87dff']}) }
...
function seed() {
  ...
  contacts.value=[
    {id:uid(),name:'Sarah Chen',title:'VP Product',company:'Orion Health',email:'sarah@orion.com',industry:'Technology',metAt:'SaaS Summit NYC',rating:'hot',hibernated:false,createdAt:ago(28),activities:[
      {id:uid(),type:'email',label:'Email',date:ago(15),note:'"Hi Sarah — great meeting you at SaaS Summit. Would love to grab 20 mins to pick your brain on the Orion roadmap."',isResponse:false},
      {id:uid(),type:'linkedin',label:'LinkedIn',date:ago(27),note:'Connected on LinkedIn — sent a personalised note referencing the AI in healthcare conversation.',isResponse:true,responseNote:'Accepted within the hour'},
      {id:uid(),type:'meeting',label:'Meeting',date:ago(28),note:'Met at SaaS Summit NYC. Really aligned on AI in healthcare workflows. She mentioned Q3 budget conversations.',isResponse:true,responseNote:'Great conversation'},
    ]},
    {id:uid(),name:'Marcus Webb',title:'MD',company:'Meridian Capital',email:'mwebb@meridian.com',industry:'Finance',metAt:'Meridian Investor Dinner',rating:'hot',hibernated:false,createdAt:ago(19),activities:[
      ...
      {id:uid(),type:'email',label:'Email',date:ago(16),note:'"Marcus — great connecting at the dinner. Sending over the one-pager you asked for."',isResponse:true,responseNote:'Replied same day'},
      {id:uid(),type:'meeting',label:'Meeting',date:ago(19),note:'Met at Meridian Investor Dinner. MD, knows everyone in the room. Incredibly warm.',isResponse:true,responseNote:'Great energy'},
    ]},
    {id:uid(),name:'James Okafor',title:'Partner',company:'Vantage Law',industry:'Legal',metAt:'Networking Breakfast',rating:'warm',hibernated:false,createdAt:ago(26),activities:[
      {id:uid(),type:'email',label:'Email',date:ago(6),note:'"James — happy to connect you with the Orion team if useful."',isResponse:false},
      {id:uid(),type:'text',label:'Text',date:ago(12),note:'Sent a congrats text on the Vantage announcement.',isResponse:true,responseNote:"Thanks! Let's grab coffee"},
      {id:uid(),type:'meeting',label:'Meeting',date:ago(26),note:'Met at Networking Breakfast. Partner at Vantage Law — M&A and corporate.',isResponse:true,responseNote:'Solid connection'},
    ]},
    {id:uid(),name:'Diana Russo',title:'CEO',company:'Flux Studio',industry:'Design',metAt:"Founders Dinner",rating:'nurture',hibernated:false,createdAt:ago(30),activities:[
      {id:uid(),type:'text',label:'Text',date:ago(4),note:'Quick congrats text on the Flux Studio rebrand.',isResponse:true,responseNote:'Thanks! Love that you noticed 🙏'},
      {id:uid(),type:'meeting',label:'Meeting',date:ago(30),note:'Met at Founders Dinner. CEO of Flux Studio. Thoughtful on brand and culture. Long game.',isResponse:true,responseNote:'Warm connection'},
    ]},
    {id:uid(),name:'Priya Mehta',title:'Director',company:'WestBridge RE',industry:'Real Estate',metAt:'RE Investors Forum',rating:'cold',hibernated:false,createdAt:ago(22),activities:[
      {id:uid(),type:'email',label:'Email',date:ago(18),note:'"Priya — following up from the RE Investors Forum. Happy to share data on commercial trends."',isResponse:false},
      {id:uid(),type:'meeting',label:'Meeting',date:ago(22),note:'Met at RE Investors Forum. Polite but guarded.',isResponse:false},
    ]},
    {id:uid(),name:'Noel Patterson',title:'Partner',company:'Criterion Ventures',industry:'Venture Capital',metAt:'VC Summit',rating:'cold',hibernated:true,hibernatedAt:ago(6),createdAt:ago(45),activities:[
      {id:uid(),type:'email',label:'Email',date:ago(38),note:"Follow-up after VC Summit.",isResponse:false},
    ]},
  ]
  xp.value={...xp.value,totalXP:1340,level:7,streak:7,lastActivityDate:todayStr(),totalScans:5,totalContacts:6,unlockedBadges:['card_shark','hot_streak','networker'],completedMissions:['scan','followup'],missionsDate:todayStr()}
  save()
}
...
<div class="cd-root">
  ...
  <div class="cd-phone-col">
    <div class="cd-phone">
      ...
      <div class="cd-inner">
        <div class="cd-sbar"><span>9:41</span><span style="font-family:var(--cd-mono);font-size:11px">Card<span style="color:var(--cd-accent)">Desk</span></span></div>

        <!-- ══ VIBE FEED ══ -->
        <div class="cd-screen" :class="{on:screen==='vibe'}">
          <div class="cd-shdr"><div class="cd-stitle">Your Vibe ⚡</div><div class="cd-ssub">What's happening in your world right now</div></div>
          <div class="cd-scrl cd-pad">
            <!-- Session entry -->
            <div class="cd-sess-entry" @click="nav('session')">
              <div class="cd-se-top"><span style="font-size:26px">🎙</span><div><div class="cd-se-ttl">Need a session?</div><div class="cd-se-sub">30 seconds. You'll feel better. Promise.</div></div></div>
            ...
            <div class="cd-vc hype" @click="fireConfetti();earnXP('log_followup')">
              <div class="cd-vct"><span class="cd-vci">🏆</span><div style="flex:1"><div class="cd-vch" style="color:var(--cd-accent)">Nobody crushes it like you.</div><div class="cd-vcb">{{ contacts.length }} contacts in your network, {{ xp.streak }}-day streak. <strong>You're building something real here.</strong></div></div><span class="cd-xpb">+20 XP</span></div>
              <button class="cd-abtn g">🤘 Claim your glory</button>
            </div>
            <!-- Cold warmer -->
            <div v-if="coldCs.length" class="cd-vc cold-vc" @click="nav('cold')">
              <div class="cd-vct"><span class="cd-vci">❄️</span><div style="flex:1"><div class="cd-vch" style="color:var(--cd-ice)">{{ coldCs[0].name }} has gone quiet.</div><div class="cd-vcb"><em style="font-style:normal;color:var(--cd-ice)">Sometimes the slow burns lead to the best leads.</em> One check-in could be all it takes.</div></div></div>
            ...
            <div v-if="alertCs.length" class="cd-vc warn" @click="goDetail(alertCs[0].id)">
              <div class="cd-vct"><span class="cd-vci">⚡</span><div style="flex:1"><div class="cd-vch" style="color:var(--cd-orange)">{{ alertCs[0].name }} is slipping away.</div><div class="cd-vcb">{{ daysSinceLast(alertCs[0]) }} days without a follow-up.<span v-if="alertCs[0].rating==='hot'"> They were 🔥 Hot. That window is still open.</span></div></div></div>
            ...
            <div class="cd-mood" @click="moodIdx++">
              ...
              <div class="cd-mc-t" :style="`color:var(--cd-${curMood.color})`">{{ curMood.title }}</div>
            ...
            <div class="cd-vc tip-vc">
              <div class="cd-vct"><span class="cd-vci">💡</span><div style="flex:1"><div class="cd-vch" style="color:var(--cd-purple)">Pro tip: the 48-hour rule</div><div class="cd-vcb">Follow up within 48 hours of meeting someone and you're <strong>5x more likely</strong> to get a response.</div></div></div>
              <div style="display:flex;gap:5px;flex-wrap:wrap"><span class="cd-tip-tag">⏰ 48hrs</span><span class="cd-tip-tag">📧 Personal email</span><span class="cd-tip-tag">🎯 Reference the convo</span></div>
            </div>
            <div class="cd-vc tip-vc">
              <div class="cd-vct"><span class="cd-vci">😴</span><div style="flex:1"><div class="cd-vch" style="color:var(--cd-purple)">Hibernate, don't delete.</div><div class="cd-vcb">CardDesk is built on abundance. Nobody gets deleted — just paused. <strong>The person who ghosted you in Q1 might be your best intro in Q3.</strong></div></div></div>
          ...
          <nav class="cd-bnav">
            ...
            <button class="cd-bn" @click="nav('contacts')" style="position:relative"><span v-if="alertCs.length" class="cd-nav-dot"></span><span class="cd-bni">👥</span>Network</button>
          </nav>
        </div>
        ...
        <div class="cd-screen" :class="{on:screen==='session'}">
          <div class="cd-scrl cd-pad" style="background:radial-gradient(ellipse 80% 50% at 50% 0%,rgba(0,255,135,0.06) 0%,transparent 60%)">
            <div style="text-align:center;padding:12px 0 16px">
              ...
              <div style="font-family:var(--cd-bebas);font-size:46px;letter-spacing:1px;line-height:1;margin-bottom:6px">Need a session?</div>
              <div style="font-size:12px;color:var(--cd-muted);line-height:1.6">Pick your vibe. All you need is one text.</div>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:15px">
              <div class="cd-mcard tg" :class="{sel:sessionMode==='tough'}" @click="sessionMode='tough'"><div style="font-size:28px;margin-bottom:6px">💪</div><div class="cd-mc-lbl">Need a talking to</div><div class="cd-mc-sub">Real talk. No fluff. Time to move.</div></div>
              <div class="cd-mcard pk" :class="{sel:sessionMode==='hype'}" @click="sessionMode='hype'"><div style="font-size:28px;margin-bottom:6px">🏆</div><div class="cd-mc-lbl">Picker upper</div><div class="cd-mc-sub">Pure hype. Your numbers say it all.</div></div>
            </div>
            <Transition name="cd-pop">
              <div v-if="sessionMode==='tough'" class="cd-scard tc">
                ...
                <div class="cd-sc-b tc" v-html="curTough.b"></div>
                <button class="cd-abtn o" @click="earnXP('log_followup');toughIdx++">✉ Send one text now <span class="cd-xpb" style="background:rgba(0,0,0,0.2);border:none;color:#fff8">+25 XP</span></button>
            ...
            <Transition name="cd-pop">
              <div v-if="sessionMode==='hype'" class="cd-scard hc">
                ...
                <div class="cd-sc-b hc" v-html="curHype.b"></div>
                <button class="cd-abtn g" @click="earnXP('log_followup');hypeIdx++">🚀 Log a touchpoint <span class="cd-xpb" style="background:rgba(0,0,0,0.15);border:none;color:rgba(0,0,0,0.65)">+25 XP</span></button>
                <div class="cd-nxt" @click="hypeIdx++">Another one →</div>
              </div>
            </Transition>
            <div v-if="sessionMode" class="cd-lucky"><div style="font-size:18px;margin-bottom:5px">✨</div><div style="font-family:var(--cd-cond);font-size:15px;font-weight:800;color:var(--cd-accent);margin-bottom:4px">Remember</div><div style="font-size:12px;color:var(--cd-muted);line-height:1.6"><em style="font-style:normal;color:var(--cd-text)">They are the lucky ones to hear from you.</em><br>You reaching out is a gift. Own it.</div></div>
          </div>
          <nav class="cd-bnav">
        ...
        <div class="cd-screen" :class="{on:screen==='cold'}">
          <div class="cd-shdr"><div class="cd-stitle">❄️ Cold Contacts</div><div class="cd-ssub">Slow burns can still catch fire.</div></div>
          <div class="cd-scrl cd-pad">
            <div style="background:linear-gradient(135deg,rgba(168,216,234,0.07),rgba(77,166,255,0.04));border:1px solid rgba(168,216,234,0.18);border-radius:13px;padding:11px 13px;margin-bottom:12px;display:flex;align-items:center;gap:10px">
              <span style="font-size:22px;flex-shrink:0">🌡</span>
              <div><div style="font-family:var(--cd-cond);font-size:14px;font-weight:800;color:var(--cd-ice);margin-bottom:2px">The Slow Burn Section</div><div style="font-size:11px;color:var(--cd-muted);line-height:1.5">Tap any contact for a warm-up nudge. Hibernate instead of delete.</div></div>
            </div>
            <div v-if="!coldCs.length&&!hibCs.length" class="cd-empty"><div style="font-size:40px;margin-bottom:10px">❄️</div><div style="font-family:var(--cd-cond);font-size:18px;font-weight:800;margin-bottom:6px">No cold contacts yet</div><div style="font-size:12px;color:var(--cd-muted)">Rate a contact ❄️ Cold and they'll appear here.</div></div>
            <template v-if="coldCs.length">
              ...
              <div v-for="c in coldCs" :key="c.id" class="cd-cold-card" :class="{open:openCold.includes(c.id)}">
                <div class="cd-cc-top" @click="toggleCold(c.id)"><div class="cd-cc-av">{{ cEmoji(c) }}</div><div style="flex:1;min-width:0"><div class="cd-cc-nm">{{ c.name }}</div><div class="cd-cc-sb">{{ [c.title,c.company].filter(Boolean).join(' · ') }}</div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;flex-shrink:0"><span class="cd-cpill">❄️ Cold</span><span class="cd-dlbl" v-if="daysSinceLast(c)">{{ daysSinceLast(c) }}d ago</span></div></div>
                <Transition name="cd-expand">
                  <div v-if="openCold.includes(c.id)" class="cd-cw">
                    <div class="cd-cw-q">"<em style="font-style:normal;color:var(--cd-ice)">{{ coldWarmer(c) }}</em>"</div>
                    <div v-if="c.metAt" style="font-size:10px;color:var(--cd-dim);margin-bottom:8px">Met @ {{ c.metAt }}</div>
                    <div style="display:flex;gap:6px"><button class="cd-cwb reach" @click="earnXP('log_followup');toggleCold(c.id)">📧 Reach out +25 XP</button><button class="cd-cwb hib" @click="hibernate(c.id)">Hibernate 😴</button></div>
                  </div>
                </Transition>
              </div>
            </template>
            <template v-if="hibCs.length">
              ...
              <div v-for="c in hibCs" :key="c.id" class="cd-cold-card" style="opacity:0.65" :class="{open:openCold.includes(c.id+'_h')}">
                <div class="cd-cc-top" @click="toggleCold(c.id+'_h')"><div class="cd-cc-av" style="filter:grayscale(0.5)">{{ cEmoji(c) }}</div><div style="flex:1;min-width:0"><div class="cd-cc-nm">{{ c.name }}</div><div class="cd-cc-sb">{{ [c.title,c.company].filter(Boolean).join(' · ') }}</div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;flex-shrink:0"><span class="cd-hpill">😴 Hibernating</span><span class="cd-dlbl" v-if="c.hibernatedAt">since {{ fmtShort(c.hibernatedAt) }}</span></div></div>
                <Transition name="cd-expand">
                  <div v-if="openCold.includes(c.id+'_h')" class="cd-cw">
                    ...
                    <div style="display:flex;gap:6px;margin-top:8px"><button class="cd-cwb wake" @click="wake(c.id);toggleCold(c.id+'_h')">Wake up 🌅 +10 XP</button></div>
                  </div>
                </Transition>
              </div>
            </template>
        ...
        <div class="cd-screen" :class="{on:screen==='home'}">
          <div class="cd-scrl cd-pad">
            ...
            <div class="cd-hero">
              ...
              <div style="font-family:var(--cd-bebas);font-size:42px;line-height:1;letter-spacing:1px">{{ curLvl.title }}</div>
              <div style="font-size:11px;color:var(--cd-muted);font-weight:600;text-transform:uppercase;letter-spacing:1px;margin-bottom:13px">Level {{ xp.level }} · {{ xp.totalXP }} XP total</div>
              <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-size:10px;font-weight:700;color:var(--cd-dim);text-transform:uppercase;letter-spacing:0.8px">XP to {{ nextLvl?.title||'Max' }}</span><span style="font-family:var(--cd-mono);font-size:11px;color:var(--cd-accent)">{{ xp.totalXP }} / {{ nextLvl?.xp||'∞' }}</span></div>
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:11px">
              <div class="cd-stat" @click="nav('contacts')"><div class="cd-stat-n" style="color:var(--cd-accent)">{{ contacts.length }}</div><div class="cd-stat-l">Contacts</div></div>
              <div class="cd-stat" @click="nav('contacts')"><div class="cd-stat-n" style="color:var(--cd-orange)">🔥{{ hotCount }}</div><div class="cd-stat-l">Hot Leads</div></div>
              <div class="cd-stat" @click="nav('contacts')"><div class="cd-stat-n" style="color:var(--cd-yellow)">{{ alertCs.length }}</div><div class="cd-stat-l">Overdue</div></div>
            </div>
            <!-- Streak -->
            <div class="cd-streak">
              <div style="font-size:34px;animation:cd-wig 1.8s ease-in-out infinite;flex-shrink:0">🔥</div>
              <div style="flex:1"><div style="font-family:var(--cd-bebas);font-size:42px;color:var(--cd-fire);line-height:1;letter-spacing:-1px">{{ xp.streak }}</div><div style="font-size:11px;font-weight:700;color:var(--cd-muted);text-transform:uppercase;letter-spacing:0.8px">Day Streak</div><div style="font-size:10px;color:var(--cd-dim);margin-top:1px;font-style:italic">{{ xp.streak>=7?'You show up. Every. Single. Day. 🤘':xp.streak>0?`${7-xp.streak} more days to 200 XP bonus`:'Start today — log anything.' }}</div></div>
              <div><div style="display:flex;gap:4px;margin-bottom:3px"><div v-for="(d,i) in sDots" :key="i" class="cd-sdot" :class="d"></div></div><div style="font-size:8px;color:var(--cd-dim);text-align:center;font-weight:700;letter-spacing:0.5px">MON → SUN</div></div>
            </div>
            <!-- Missions -->
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:9px"><div style="font-family:var(--cd-bebas);font-size:20px;letter-spacing:1px;color:var(--cd-muted)">Daily Missions</div><div style="font-size:11px;color:var(--cd-accent);font-weight:700">{{ xp.completedMissions.length }} / {{ MISSIONS.length }}</div></div>
            <div v-for="m in MISSIONS" :key="m.key" class="cd-mission" :class="{done:xp.completedMissions.includes(m.key)}" @click="doMission(m.key)">
              ...
              <div style="flex:1"><div style="font-size:13px;font-weight:700">{{ m.label }}</div><div style="font-size:10px;color:var(--cd-dim);margin-top:1px;font-style:italic">{{ m.hype }}</div></div>
              <span v-if="!xp.completedMissions.includes(m.key)" class="cd-xpb">+{{ m.xp }} XP</span>
              <span v-else style="font-size:20px">✅</span>
            </div>
            <!-- Badges -->
            <div style="display:flex;justify-content:space-between;align-items:center;margin:8px 0 9px"><div style="font-family:var(--cd-bebas);font-size:20px;letter-spacing:1px;color:var(--cd-muted)">Badges</div><div style="font-size:11px;color:var(--cd-accent);font-weight:700">{{ xp.unlockedBadges.length }} unlocked</div></div>
        ...
        <div class="cd-screen" :class="{on:screen==='contacts'}">
          <div class="cd-shdr" style="padding-bottom:8px">
            ...
            <div style="position:relative;margin-bottom:8px"><span style="position:absolute;left:11px;top:50%;transform:translateY(-50%);color:var(--cd-dim);font-size:15px">⌕</span><input v-model="cSearch" class="cd-inp" style="padding-left:32px" placeholder="Search contacts..." /></div>
            <div style="display:flex;gap:5px;overflow-x:auto;padding-bottom:2px">
              ...
              <button v-for="r in RATINGS" :key="r.key" class="cd-pill" :class="[{on:cFilter===r.key},r.key]" @click="cFilter=cFilter===r.key?'':r.key">{{ r.emoji }} {{ r.label }}</button>
              <select v-model="cSort" class="cd-sort-sel"><option value="recent">Recent</option><option value="hot">🔥 Hottest</option><option value="name">A–Z</option></select>
            </div>
          </div>
          <div class="cd-scrl" style="padding:4px 14px 8px">
            <div v-if="!contacts.length" class="cd-empty"><div style="font-size:40px;margin-bottom:10px">🃏</div><div style="font-family:var(--cd-cond);font-size:18px;font-weight:800;margin-bottom:6px">No contacts yet</div><div style="font-size:12px;color:var(--cd-muted);margin-bottom:14px">Tap Add to get started.</div><button class="cd-abtn g" @click="nav('add')">📷 Add First Contact</button></div>
            <div v-for="c in filteredCs" :key="c.id" class="cd-crd" :class="{alrt:needsFU(c)}" @click="goDetail(c.id)">
              ...
              <div style="flex:1;min-width:0"><div class="cd-cnm">{{ c.name }}</div><div class="cd-csb">{{ [c.title,c.company].filter(Boolean).join(' · ') }}</div><div v-if="lastAct(c)" class="cd-cmeta">{{ getAct(lastAct(c)!.type).icon }} {{ lastAct(c)!.label }} · {{ fmtShort(lastAct(c)!.date) }}</div></div>
          ...
          <template v-if="selContact">
            ...
            <template v-if="editing">
              <div class="cd-scrl cd-pad">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><div><label class="cd-lbl">Title</label><input v-model="editForm.title" class="cd-inp" /></div><div><label class="cd-lbl">Company</label><input v-model="editForm.company" class="cd-inp" /></div></div>
                <label class="cd-lbl">Email</label><input v-model="editForm.email" type="email" class="cd-inp" />
                <label class="cd-lbl">Rating</label>
                <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px"><button v-for="r in RATINGS" :key="r.key" class="cd-rpick" :style="editForm.rating===r.key?`background:${r.color}22;border-color:${r.color};color:${r.color}`:''" @click="editForm.rating=editForm.rating===r.key?'':r.key">{{ r.emoji }} {{ r.label }}</button></div>
                ...
                <div style="display:flex;gap:8px;margin-top:4px"><button class="cd-abtn g" style="font-size:13px;padding:10px" @click="saveEdit">Save</button><button class="cd-abtn" style="background:transparent;color:var(--cd-muted);border-color:var(--cd-bdr);font-size:13px;padding:10px" @click="editing=false">Cancel</button></div>
              </div>
            </template>
            <!-- VIEW MODE -->
            <template v-else>
              <div class="cd-scrl cd-pad">
                ...
                <div class="cd-det-hero">
                  <div style="display:flex;align-items:center;gap:11px;margin-bottom:10px">
                    ...
                    <div><div style="font-family:var(--cd-bebas);font-size:26px;letter-spacing:0.5px;line-height:1;margin-bottom:3px">{{ selContact.name }}</div><div style="font-size:12px;color:var(--cd-muted);font-weight:600">{{ [selContact.title,selContact.company].filter(Boolean).join(' · ') }}</div></div>
                  </div>
                ...
                <div class="cd-fu-banner" :class="fuStatus(selContact)">
                  ...
                  <div><div class="cd-fu-t">{{ fuInfo(selContact).title }}</div><div class="cd-fu-s">{{ fuInfo(selContact).sub }}</div></div>
                </div>
                <!-- Info grid -->
                <div v-if="selContact.email||selContact.phone" class="cd-info-grid">
                  <div v-if="selContact.email" class="cd-info-row"><span class="cd-info-k">📧 Email</span><a :href="`mailto:${selContact.email}`" class="cd-info-v link">{{ selContact.email }}</a></div>
                  <div v-if="selContact.phone" class="cd-info-row"><span class="cd-info-k">📞 Phone</span><a :href="`tel:${selContact.phone}`" class="cd-info-v link">{{ selContact.phone }}</a></div>
                </div>
                <div v-if="selContact.notes" class="cd-note-blk"><div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:var(--cd-dim);margin-bottom:4px">Notes</div>{{ selContact.notes }}</div>
                <!-- Log touchpoint -->
                <div class="cd-log-sec">
                  ...
                  <div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px">
                    <button v-for="t in ACT_TYPES" :key="t.key" class="cd-act-type" :class="{sel:actType===t.key}" @click="actType=t.key"><span style="font-size:14px;display:block;margin-bottom:2px">{{ t.icon }}</span>{{ t.label }}</button>
                  </div>
                ...
                <div style="margin-bottom:14px">
                  <div style="font-family:var(--cd-cond);font-size:13px;font-weight:800;text-transform:uppercase;letter-spacing:0.8px;color:var(--cd-dim);margin-bottom:12px;display:flex;align-items:center;gap:8px">Activity Timeline <div style="flex:1;height:1px;background:var(--cd-bdr)"></div></div>
                  <div v-if="!sortedActs.length" style="background:var(--cd-bg2);border:1px solid var(--cd-bdr);border-radius:12px;padding:16px;text-align:center;color:var(--cd-dim);font-size:12px">No activity yet — log your first touchpoint above.</div>
                  <div v-for="(act,i) in sortedActs" :key="act.id" style="display:flex;gap:9px;margin-bottom:13px;position:relative">
                    ...
                    <div style="flex:1;background:var(--cd-bg2);border:1px solid var(--cd-bdr);border-radius:12px;padding:10px 12px">
                      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px"><div style="font-family:var(--cd-cond);font-size:14px;font-weight:800;letter-spacing:0.3px">{{ act.label }}</div><div style="font-family:var(--cd-mono);font-size:10px;color:var(--cd-dim)">{{ formatDate(act.date) }}</div></div>
                      <div v-if="act.note" style="font-size:12px;color:var(--cd-muted);line-height:1.5;margin-bottom:7px">{{ act.note }}</div>
                      <div class="cd-tl-resp" :class="act.isResponse?'yes':'no'" @click="!act.isResponse&&toggleResp(act.id)">{{ act.isResponse?`✓ ${act.responseNote||'Responded'}`:'○ No reply yet — tap to mark responded' }}</div>
                    </div>
                  </div>
                  <div style="background:var(--cd-bg2);border:1px solid var(--cd-bdr);border-radius:12px;padding:12px 14px"><div style="font-family:var(--cd-cond);font-size:14px;font-weight:800;color:var(--cd-muted);margin-bottom:5px">⏰ 7-Day Follow-Up Rule</div><div style="font-size:11px;color:var(--cd-dim);line-height:1.6">No reply after <strong style="color:var(--cd-muted)">7 days</strong> → surfaced in Vibe Feed. After <strong style="color:var(--cd-orange)">10 days</strong> → Overdue alert fires. 🔥 Hot contacts always surface first.</div></div>
                </div>
                <!-- Actions -->
        ...
        <div class="cd-screen" :class="{on:screen==='add'}">
          <div class="cd-shdr"><button class="cd-back" style="margin-bottom:6px" @click="nav('contacts')">← Back</button><div class="cd-stitle">Add Contact</div><div class="cd-ssub">Scan or enter manually · <span style="color:var(--cd-accent)">+50 XP</span> on scan</div></div>
          <div class="cd-scrl cd-pad">
            ...
            <div style="display:flex;align-items:center;gap:10px;color:var(--cd-dim);font-size:10px;margin:12px 0;text-transform:uppercase;letter-spacing:1px;font-weight:700"><div style="flex:1;height:1px;background:var(--cd-bdr)"></div>or enter manually<div style="flex:1;height:1px;background:var(--cd-bdr)"></div></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><div><label class="cd-lbl">First Name</label><input v-model="addForm.firstName" class="cd-inp" placeholder="Jane" /></div><div><label class="cd-lbl">Last Name</label><input v-model="addForm.lastName" class="cd-inp" placeholder="Smith" /></div></div>
            ...
            <select v-model="addForm.industry" class="cd-inp" style="cursor:pointer"><option value="">Select...</option><option v-for="ind in INDUSTRIES" :key="ind" :value="ind">{{ ind }}</option></select>
            ...
            <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px"><button v-for="r in RATINGS" :key="r.key" class="cd-rpick" :style="addForm.rating===r.key?`background:${r.color}22;border-color:${r.color};color:${r.color}`:''" @click="addForm.rating=addForm.rating===r.key?'':r.key">{{ r.emoji }} {{ r.label }}</button></div>
            <button class="cd-abtn g" style="font-size:16px;padding:13px;margin-top:4px" @click="saveContact" :disabled="!addName">SAVE + EARN 25 XP →</button>
          </div>
          <nav class="cd-bnav">
            <button class="cd-bn" @click="nav('vibe')"><span class="cd-bni">⚡</span>Vibe</button>
            <button class="cd-bn" @click="nav('session')"><span class="cd-bni">🎙</span>Session</button>
  ...
  <div class="cd-panel">
    ...
    <div style="font-size:13px;color:var(--cd-dim);font-weight:600;margin-bottom:22px">Interactive Prototype · 6 Screens · All data persists in localStorage</div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:22px">
      <button v-for="({k,icon,label}) in [{k:'vibe',icon:'⚡',label:'Vibe'},{k:'session',icon:'🎙',label:'Session'},{k:'cold',icon:'❄️',label:'Cold'},{k:'home',icon:'🏠',label:'Dashboard'},{k:'contacts',icon:'👥',label:'Network'},{k:'add',icon:'📷',label:'Add'}]"
        :key="k" class="cd-tab" :class="{on:screen===k}" @click="nav(k as Screen)">{{ icon }} {{ label }}
      </button>
    </div>
    ...
    <div class="cd-guide-grid">
      <div v-for="s in [{icon:'⚡',name:'Vibe Feed',tag:'LANDING',tc:'var(--cd-accent)',desc:'Live hype cards, cold warmers, overdue alerts, session entry, rotating mood card, tips. All pull from real contact data.'},{icon:'🎙',name:'Session',tag:'NEW',tc:'var(--cd-purple)',desc:'Talking to (tough love) or Picker upper (hype). 3 rotating rounds each. Anchored by the "lucky ones" reminder.'},{icon:'❄️',name:'Cold',tag:'NEW',tc:'var(--cd-ice)',desc:'Cold contacts with personalised warm-up copy. Expand to reveal the warmer message. Hibernate instead of delete.'},{icon:'🏠',name:'Dashboard',tag:'CORE',tc:'var(--cd-dim)',desc:'XP bar, level, 7-day streak with fire animation, daily missions, badge shelf. All live data.'},{icon:'👥',name:'Network',tag:'CORE',tc:'var(--cd-dim)',desc:'Filtered contact list. Tap any contact to open the full detail view with activity timeline and log form.'},{icon:'📷',name:'Add',tag:'CORE',tc:'var(--cd-dim)',desc:'Manual form + AI scan zone with confetti. Rating picker. Saves to localStorage instantly. +50 XP on scan.'}]"
        :key="s.name" class="cd-guide-card">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span style="font-size:18px">{{ s.icon }}</span><div><div style="font-family:var(--cd-cond);font-size:15px;font-weight:800;letter-spacing:0.3px">{{ s.name }}</div><div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.8px" :style="`color:${s.tc}`">{{ s.tag }}</div></div></div>
    ...
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:22px">
      <div v-for="(w,i) in [['❄️',"It's a bummer, but maybe they just need time to thaw out."],['🕯','Sometimes the slow burns lead to the best leads — don\'t give up.'],['🌍','Maybe they\'re just busy — remind them how you can enhance their world.'],['🌅','Welcome back — timing is everything, and yours might be perfect right now.']]"
        :key="i" class="cd-warmer-card">
        <span style="font-size:18px;flex-shrink:0">{{ w[0] }}</span>
        <div style="font-family:var(--cd-cond);font-size:14px;font-weight:800;color:var(--cd-ice);line-height:1.3">"{{ w[1] }}"</div>
      </div>
    </div>
    <div class="cd-divider"></div>
    <!-- XP table -->
    ...
    <div class="cd-xp-table">
      <div v-for="([action,pts]) in [['📷 Scan a card','50 XP'],['✉️ Log follow-up','25 XP'],['🎉 Got a response','100 XP'],['🔥 Follow up Hot lead','50 XP'],['🎯 Complete mission','50 XP'],['🔥 7-day streak bonus','200 XP'],['🌉 Make an intro','150 XP'],['🏅 Unlock badge','75 XP']]"
        :key="action" class="cd-xp-row">
        <span style="font-size:13px;color:var(--cd-muted)">{{ action }}</span>
        <span style="font-family:var(--cd-mono);font-size:12px;color:var(--cd-accent);font-weight:700">{{ pts }}</span>
      </div>
    </div>
  </div><!-- /cd-panel -->
...
.cd-root {
  ...
  --cd-bebas:'Bebas Neue',sans-serif;--cd-cond:'Barlow Condensed',sans-serif;
  --cd-sans:'Barlow',sans-serif;--cd-mono:'JetBrains Mono',monospace;
...
.cd-inp   { width:100%;background:var(--cd-bg2);border:1px solid var(--cd-bdr);border-radius:10px;color:var(--cd-text);font-family:var(--cd-sans);font-size:13px;padding:9px 12px;outline:none;transition:border-color 0.15s;margin-bottom:8px; }
...
.cd-sc-eye.orange { color:var(--cd-orange); } .cd-sc-eye.green { color:var(--cd-accent); }
.cd-sc-q   { font-family:var(--cd-cond);font-size:21px;font-weight:800;line-height:1.25;margin-bottom:9px; }
.cd-sc-b   { font-size:12px;color:var(--cd-muted);line-height:1.7;margin-bottom:12px; }
.cd-sc-b.tc :deep(em) { font-style:normal;color:var(--cd-orange); }
.cd-sc-b.tc :deep(strong) { color:var(--cd-text); }
.cd-sc-b.hc :deep(em) { font-style:normal;color:var(--cd-accent); }
.cd-sc-b.hc :deep(strong) { color:var(--cd-text); }
...
.cd-cbar.hot { background:var(--cd-orange); } .cd-cbar.warm { background:var(--cd-yellow); }
.cd-cbar.nurture { background:var(--cd-accent); } .cd-cbar.cold { background:var(--cd-blue); }
...
.cd-info-v.link { color:var(--cd-blue);text-decoration:none; }

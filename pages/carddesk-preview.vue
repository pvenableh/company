...
const BADGES = [
	...
	},
	{
		...
		check: (s: any) => s.totalContacts >= 10,
	},
	{
		key: 'dealmaker',
		emoji: '💎',
...
const VIBE_MOODS = [
	{
		e: '😶‍🌫️',
		title: 'Feeling antisocial lately?',
		color: 'blue',
		body: "We get it. One message. You don't have to be 'on' — just human. That's always enough.",
	},
	{
		e: '🔥',
		title: 'You are absolutely killing it.',
		color: 'green',
		body: "Contacts growing, streak alive. You're not just networking — you're building a career asset.",
	},
	{
		e: '😴',
		title: 'A little tired today?',
		color: 'blue',
		body: 'A 2-second LinkedIn reaction keeps the connection warm and saves your streak. Minimum effort, maximum effect.',
	},
	{
		e: '🏆',
		title: 'You are brilliant at this.',
		color: 'green',
		body: "You show up, follow through, make people feel seen. That's not the app — that's you being exceptional.",
	},
	{
		e: '🛋️',
		title: 'Introverted this week?',
		color: 'purple',
		body: 'Pick your 3 hottest leads. One message each. Done in 5 minutes.',
	},
];
...
const PANEL_SCREENS = [
	{
		...
		tag: 'LANDING',
		tc: 'var(--cd-accent)',
		desc: 'Live hype cards, cold warmers, overdue alerts, session entry, rotating mood card, and tips. All pull from real contact data.',
	},
	{
		icon: '🎙',
		name: 'Session',
		tag: 'NEW',
		tc: 'var(--cd-purple)',
		desc: 'Talking to (tough love) or Picker upper (hype). 3 rotating rounds each. Anchored by the lucky ones reminder.',
	},
	{
		icon: '❄️',
		name: 'Cold',
		tag: 'NEW',
		tc: 'var(--cd-ice)',
		desc: 'Cold contacts with personalised warm-up copy. Tap to expand. Hibernate instead of delete.',
	},
	{
		icon: '🏠',
		name: 'Dashboard',
		tag: 'CORE',
		tc: 'var(--cd-dim)',
		desc: 'XP bar, level, 7-day streak with fire animation, daily missions, badge shelf. All live data.',
	},
	{
		icon: '👥',
		name: 'Network',
		tag: 'CORE',
		tc: 'var(--cd-dim)',
		desc: 'Filtered contact list. Tap any contact to open the full detail view with activity timeline and log form.',
	},
	{
		icon: '📷',
		name: 'Add',
		tag: 'CORE',
		tc: 'var(--cd-dim)',
		desc: 'Manual form + AI scan zone with confetti. Rating picker. Saves to localStorage instantly. +50 XP on scan.',
	},
];
const PANEL_WARMERS = [
	{
		icon: '❄️',
		text: "It's a bummer, but maybe they just need time to thaw out. That doesn't mean the connection isn't real.",
	},
	{
		icon: '🕯',
		text: "Sometimes the slow burns lead to the best leads — don't give up. Your patience is a competitive advantage.",
	},
	{
		icon: '🌍',
		text: "Maybe they're just busy — remind them how you can enhance their world. A genuine check-in goes further than you think.",
	},
	{
		icon: '🌅',
		text: "Welcome back — timing is everything, and yours might be perfect right now. You're fresh on the scene again.",
	},
];
const PANEL_XP = [
...
function daysSinceLast(c: Contact) {
	return daysSince(lastAct(c)?.date);
}
...
function needsFU(c: Contact) {
	return !c.hibernated && fuStatus(c) === 'overdue';
}
function fuInfo(c: Contact) {
	const s = fuStatus(c),
		la = lastAct(c),
		d = la ? daysSince(la.date) : null;
	const map = {
		overdue: {
			ico: '⚡',
			cls: 'overdue',
			title: `${d} days overdue — don't let this one slip`,
			sub: `Last: ${la?.label || 'Contact'} on ${formatDate(la?.date)} · ${la?.isResponse ? '✓ Replied' : 'No reply yet'}`,
		},
		due: {
			ico: '⏰',
			cls: 'due',
			title: `${d} days since last contact — right on the line`,
		...
		ok: {
			...
			sub: `Last: ${la?.label || 'Contact'} on ${formatDate(la?.date)} · ${la?.isResponse ? '✓ Replied' : 'Awaiting reply'}`,
		},
		new: {
			ico: '👋',
			cls: 'new',
			title: 'No activity logged yet',
			sub: 'Use the form below to log your first touchpoint.',
		},
	};
	return map[s];
}
...
function load() {
	if (typeof window === 'undefined') return;
	try {
		const c = localStorage.getItem('cd2_c');
		if (c) contacts.value = JSON.parse(c);
	} catch {}
	try {
		const x = localStorage.getItem('cd2_x');
		if (x) xp.value = { ...xp.value, ...JSON.parse(x) };
	} catch {}
...
function earnXP(action: string, extra: Record<string, any> = {}) {
	const a = XP_ACTIONS[action];
	if (!a) return;
	xp.value.totalXP += a.xp;
	Object.assign(xp.value, extra);
	for (let i = LEVELS.length - 1; i >= 0; i--) {
		if (xp.value.totalXP >= LEVELS[i].xp) {
			xp.value.level = LEVELS[i].level;
			break;
		}
	}
	const today = todayStr();
...
function toggleCold(id: string) {
	openCold.value = openCold.value.includes(id) ? openCold.value.filter((x) => x !== id) : [...openCold.value, id];
}
...
function toggleResp(actId: string) {
	...
	contacts.value = contacts.value.map((c) => {
		...
		return {
			...c,
			activities: c.activities.map((a) => (a.id === actId ? { ...a, isResponse: true, responseNote: 'Responded' } : a)),
		};
	});
	save();
	earnXP('got_response');
			...
			activities: [
				{
					...
					label: 'Email',
					date: ago(15),
					note: 'Follow-up after SaaS Summit — asked about the Orion Q3 roadmap.',
					isResponse: false,
				},
				{
					id: uid(),
					...
					label: 'LinkedIn',
					date: ago(27),
					note: 'Connected on LinkedIn with a personalised note referencing the AI in healthcare conversation.',
					isResponse: true,
					responseNote: 'Accepted within the hour',
				},
				{
					id: uid(),
					type: 'meeting',
					label: 'Meeting',
					date: ago(28),
					note: 'Met at SaaS Summit NYC. Really aligned on AI in healthcare workflows. Q3 budget coming up.',
					isResponse: true,
					responseNote: 'Great conversation',
				},
			],
		},
		{
			...
			email: 'mwebb@meridian.com',
			industry: 'Finance',
			metAt: 'Meridian Investor Dinner',
			rating: 'hot',
			hibernated: false,
			createdAt: ago(19),
			activities: [
				{
					...
					label: 'Call',
					date: ago(8),
					note: 'Quick call — interested in platform overview. Said to send deck this week.',
					isResponse: true,
					responseNote: 'Send the deck',
				},
				{
					id: uid(),
					type: 'email',
					label: 'Email',
					date: ago(16),
					note: 'Sent over the one-pager he asked for at dinner.',
					isResponse: true,
					responseNote: 'Replied same day',
				},
				{
					id: uid(),
					type: 'meeting',
					label: 'Meeting',
					date: ago(19),
					note: 'Met at Meridian Investor Dinner. MD, knows everyone. Very warm energy.',
					isResponse: true,
					responseNote: 'Great energy',
				},
			],
		},
		{
			...
			activities: [
				{
					...
					label: 'Email',
					date: ago(6),
					note: 'Offered to connect him with the Orion team if useful.',
					isResponse: false,
				},
				{
					id: uid(),
					...
					label: 'Text',
					date: ago(12),
					note: 'Congrats text on the Vantage announcement.',
					isResponse: true,
					responseNote: 'Thanks! Coffee soon',
				},
				{
					id: uid(),
					type: 'meeting',
					label: 'Meeting',
					date: ago(26),
					note: 'Met at Networking Breakfast. Partner at Vantage Law M&A.',
					isResponse: true,
					responseNote: 'Solid connection',
				},
			],
		},
		{
			...
			activities: [
				{
					...
					label: 'Text',
					date: ago(4),
					note: 'Congrats text on the Flux Studio rebrand.',
					isResponse: true,
					responseNote: 'Thanks! Love that you noticed',
				},
				{
					id: uid(),
					type: 'meeting',
					label: 'Meeting',
					date: ago(30),
					note: 'Met at Founders Dinner. CEO of Flux Studio. Long game.',
					isResponse: true,
					responseNote: 'Warm connection',
				},
			],
		},
		{
			...
			name: 'Priya Mehta',
			title: 'Director',
			company: 'WestBridge RE',
			industry: 'Real Estate',
			metAt: 'RE Investors Forum',
			rating: 'cold',
			hibernated: false,
			createdAt: ago(22),
			activities: [
				{
					...
					label: 'Email',
					date: ago(18),
					note: 'Following up from the RE Investors Forum — offered to share commercial trend data.',
					isResponse: false,
				},
				{
					id: uid(),
					...
					label: 'Meeting',
					date: ago(22),
					note: 'Met at RE Investors Forum. Polite but guarded. Gave her card.',
					isResponse: false,
				},
			],
		},
			...
			activities: [
				{
					...
					label: 'Email',
					date: ago(38),
					note: 'Follow-up after VC Summit. No reply.',
					isResponse: false,
				},
			],
		},
...
<template>
	<div class="cd-root flex items-center justify-center flex-col lg:items-start lg:flex-row gap-10 lg:gap-20 p-5">
		<!-- ══ PHONE SHELL ══════════════════════════════════════════════════════════ -->
		<div class="cd-phone-col flex flex-col items-center justify-center lg:sticky top-10">
			<input
				type="checkbox"
				id="cd-menu-toggle"
				class="cd-menu-toggle mb-8 w-5 h-5"
				:checked="showDetails"
			...
			<div class="cd-phone">
				...
				<div class="cd-inner">
					<div class="cd-sbar">
						<span>9:41</span>
						<span style="font-family: var(--cd-mono); font-size: 11px">
							Card
							<span style="color: var(--cd-accent)">Desk</span>
						</span>
					</div>
					...
					<div class="cd-screen" :class="{ on: screen === 'vibe' }">
						...
						<div class="cd-scrl cd-pad">
							...
							>
								<div class="cd-vct">
									...
									<div style="flex: 1">
										...
										<div class="cd-vcb">
											{{ contacts.length }} contacts in your network, {{ xp.streak }}-day streak.
											<strong>You're building something real here.</strong>
										</div>
									</div>
							...
							<div v-if="coldCs.length" class="cd-vc cold-vc" @click="nav('cold')">
								<div class="cd-vct">
									...
									<div style="flex: 1">
										...
										<div class="cd-vcb">
											Sometimes the slow burns lead to the best leads. One check-in could be all it takes.
										</div>
									</div>
								</div>
							...
							<div v-if="alertCs.length" class="cd-vc warn" @click="goDetail(alertCs[0].id)">
								<div class="cd-vct">
									...
									<div style="flex: 1">
										...
										<div class="cd-vcb">
											{{ daysSinceLast(alertCs[0]) }} days without a follow-up.
											<span v-if="alertCs[0].rating === 'hot'">They were 🔥 Hot. That window is still open.</span>
										</div>
									</div>
								</div>
							...
							<div class="cd-mood" @click="moodIdx++">
								...
								<div style="font-size: 10px; color: var(--cd-dim); margin-top: 7px; font-family: var(--cd-mono)">
									tap to rotate
								</div>
							</div>
							...
							<div class="cd-vc tip-vc">
								<div class="cd-vct">
									...
									<div style="flex: 1">
										...
										<div class="cd-vcb">
											Follow up within 48 hours of meeting someone and you're
											<strong>5x more likely</strong>
											to get a response.
										</div>
									</div>
								</div>
							...
							<div class="cd-vc tip-vc">
								<div class="cd-vct">
									...
									<div style="flex: 1">
										...
										<div class="cd-vcb">
											Nobody gets deleted — just paused.
											<strong>The person who ghosted you in Q1 might be your best intro in Q3.</strong>
										</div>
									</div>
								</div>
					...
					<div class="cd-screen" :class="{ on: screen === 'session' }">
						<div
							class="cd-scrl cd-pad"
							style="
								background: radial-gradient(ellipse 80% 50% at 50% 0%, rgba(0, 255, 135, 0.06) 0%, transparent 60%);
							"
						>
							<div style="text-align: center; padding: 12px 0 16px">
								...
								<div style="font-size: 12px; color: var(--cd-muted); line-height: 1.6">
									Pick your vibe. All you need is one text.
								</div>
							</div>
							...
							<Transition name="cd-pop">
								<div v-if="sessionMode === 'tough'" class="cd-scard tc">
									...
									>
										...
										<span
											class="cd-xpb"
											style="background: rgba(0, 0, 0, 0.2); border: none; color: rgba(255, 255, 255, 0.8)"
										>
											+25 XP
										</span>
									</button>
							...
							<Transition name="cd-pop">
								<div v-if="sessionMode === 'hype'" class="cd-scard hc">
									...
									>
										...
										<span
											class="cd-xpb"
											style="background: rgba(0, 0, 0, 0.15); border: none; color: rgba(0, 0, 0, 0.65)"
										>
											+25 XP
										</span>
									</button>
							...
							<div v-if="sessionMode" class="cd-lucky">
								...
								<div style="font-size: 12px; color: var(--cd-muted); line-height: 1.6">
									<em style="font-style: normal; color: var(--cd-text)">They are the lucky ones to hear from you.</em>
									<br />
					...
					<div class="cd-screen" :class="{ on: screen === 'cold' }">
						...
						<div class="cd-scrl cd-pad">
							<div class="cd-cold-banner">
								...
								<div>
									<div
										...
										"
									>
										The Slow Burn Section
									</div>
									<div style="font-size: 11px; color: var(--cd-muted); line-height: 1.5">
										Tap any contact for a warm-up nudge. Hibernate instead of delete.
									</div>
								</div>
							</div>
							...
							<div v-if="!coldCs.length && !hibCs.length" class="cd-empty">
								...
								<div style="font-family: var(--cd-cond); font-size: 18px; font-weight: 800; margin-bottom: 6px">
									No cold contacts yet
								</div>
								...
								<div v-for="c in coldCs" :key="c.id" class="cd-cold-card" :class="{ open: openCold.includes(c.id) }">
									...
									<Transition name="cd-expand">
										<div v-if="openCold.includes(c.id)" class="cd-cw">
											...
											<div v-if="c.metAt" style="font-size: 10px; color: var(--cd-dim); margin-bottom: 8px">
												Met @ {{ c.metAt }}
											</div>
							...
							<template v-if="hibCs.length">
								...
								<div
									...
									class="cd-cold-card"
									style="opacity: 0.65"
									:class="{ open: openCold.includes(c.id + '_h') }"
								>
									...
									<Transition name="cd-expand">
										<div v-if="openCold.includes(c.id + '_h')" class="cd-cw">
											<div class="cd-cw-q" style="color: var(--cd-dim)">
												Smart networkers know when to pause, not delete. They always come back around.
											</div>
					...
					<div class="cd-screen" :class="{ on: screen === 'home' }">
						<div class="cd-scrl cd-pad">
							...
							<div class="cd-hero">
								...
								<div style="font-family: var(--cd-bebas); font-size: 40px; line-height: 1; letter-spacing: 1px">
									{{ curLvl.title }}
								</div>
								<div
									style="
									...
									"
								>
									Level {{ xp.level }} · {{ xp.totalXP }} XP total
								</div>
								<div style="display: flex; justify-content: space-between; margin-bottom: 4px">
									<span
										...
										"
									>
										XP to {{ nextLvl?.title || 'Max' }}
									</span>
									<span style="font-family: var(--cd-mono); font-size: 11px; color: var(--cd-accent)">
										{{ xp.totalXP }} / {{ nextLvl?.xp || '∞' }}
									</span>
								</div>
									...
									<div style="font-size: 10px; color: var(--cd-dim); margin-top: 1px; font-style: italic">
										{{
											xp.streak >= 7
												...
												: xp.streak > 0
													? 7 - xp.streak + ' more days to 200 XP bonus'
													: 'Start today — log anything.'
										}}
									</div>
								</div>
								<div>
							...
							<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 9px">
								<div style="font-family: var(--cd-bebas); font-size: 20px; letter-spacing: 1px; color: var(--cd-muted)">
									Daily Missions
								</div>
								<div style="font-size: 11px; color: var(--cd-accent); font-weight: 700">
									{{ xp.completedMissions.length }} / {{ MISSIONS.length }}
								</div>
							</div>
							<div
								v-for="m in MISSIONS"
								:key="m.key"
								class="cd-mission"
								:class="{ done: xp.completedMissions.includes(m.key) }"
							...
							<div style="display: flex; justify-content: space-between; align-items: center; margin: 8px 0 9px">
								...
								<div style="font-size: 11px; color: var(--cd-accent); font-weight: 700">
									{{ xp.unlockedBadges.length }} unlocked
								</div>
							</div>
							<div style="display: flex; gap: 7px; overflow-x: auto; padding-bottom: 4px">
								<div
									...
									:key="b.key"
									class="cd-badge"
									:class="{ ul: xp.unlockedBadges.includes(b.key) }"
									:title="b.desc"
								>
									<div
										style="font-size: 22px; margin-bottom: 3px"
										:style="xp.unlockedBadges.includes(b.key) ? '' : 'filter:grayscale(1);opacity:0.2'"
									>
										{{ b.emoji }}
									</div>
									<div
										style="
											...
											font-weight: 700;
										"
										:style="xp.unlockedBadges.includes(b.key) ? 'color:var(--cd-gold)' : 'color:var(--cd-dim)'"
									>
										{{ b.label }}
									</div>
								</div>
					...
					<div class="cd-screen" :class="{ on: screen === 'contacts' }">
						<div class="cd-shdr" style="padding-bottom: 8px">
							<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px">
								...
								<button class="cd-abtn g" style="width: auto; padding: 7px 12px; font-size: 12px" @click="nav('add')">
									📷 Add
								</button>
							</div>
							...
							<div style="display: flex; gap: 5px; overflow-x: auto; padding-bottom: 2px">
								<button class="cd-pill" :class="{ on: cFilter === '' }" @click="cFilter = ''">
									All {{ contacts.length }}
								</button>
								<button
									v-for="r in RATINGS"
						...
						<div class="cd-scrl" style="padding: 4px 14px 8px">
							...
							>
								...
								<div style="flex: 1; min-width: 0">
									...
									<div v-if="lastAct(c)" class="cd-cmeta">
										{{ getAct(lastAct(c)!.type).icon }} {{ lastAct(c)!.label }} · {{ fmtShort(lastAct(c)!.date) }}
									</div>
								</div>
								<div style="display: flex; flex-direction: column; align-items: flex-end; gap: 4px; flex-shrink: 0">
									...
									<span
										v-else-if="daysSinceLast(c)"
										style="font-family: var(--cd-mono); font-size: 10px; color: var(--cd-dim)"
									>
										{{ daysSinceLast(c) }}d
									</span>
								</div>
						...
						<template v-if="selContact">
							...
							<template v-if="editing">
								<div class="cd-scrl cd-pad">
									...
									<div style="font-family: var(--cd-cond); font-size: 18px; font-weight: 800; margin-bottom: 12px">
										Edit Contact
									</div>
									...
									<input v-model="editForm.email" type="email" class="cd-inp" />
									...
									<input v-model="editForm.metAt" class="cd-inp" />
								...
								<div class="cd-scrl cd-pad">
									...
									<div class="cd-det-hero">
										<div style="display: flex; align-items: center; gap: 11px; margin-bottom: 10px">
											...
											<div>
												...
												<div style="font-size: 12px; color: var(--cd-muted); font-weight: 600">
													{{ [selContact.title, selContact.company].filter(Boolean).join(' · ') }}
												</div>
											</div>
										</div>
									...
									<div class="cd-log-sec">
										...
										<div style="display: flex; gap: 4px; flex-wrap: wrap; margin-bottom: 8px">
											<button
												...
												@click="actType = t.key"
											>
												<span style="font-size: 14px; display: block; margin-bottom: 2px">{{ t.icon }}</span>
												{{ t.label }}
											</button>
										</div>
										<input
									...
									<div style="margin-bottom: 14px">
										...
										>
											...
											>
												...
												>
													...
													<div style="font-family: var(--cd-mono); font-size: 10px; color: var(--cd-dim)">
														{{ formatDate(act.date) }}
													</div>
												</div>
												<div
													v-if="act.note"
													style="font-size: 12px; color: var(--cd-muted); line-height: 1.5; margin-bottom: 7px"
												>
													{{ act.note }}
												</div>
												<div
													class="cd-tl-resp"
													:class="act.isResponse ? 'yes' : 'no'"
													@click="!act.isResponse && toggleResp(act.id)"
												>
													{{
														act.isResponse
															? '✓ ' + (act.responseNote || 'Responded')
															: '○ No reply yet — tap to mark responded'
													}}
												</div>
											</div>
										</div>
										...
										>
											...
											<div style="font-size: 11px; color: var(--cd-dim); line-height: 1.6">
												No reply after
												<strong style="color: var(--cd-muted)">7 days</strong>
												→ surfaced in Vibe Feed. After
												<strong style="color: var(--cd-orange)">10 days</strong>
					...
					<div class="cd-screen" :class="{ on: screen === 'add' }">
						<div class="cd-shdr">
							...
							<div class="cd-ssub">
								...
								<span style="color: var(--cd-accent)">+50 XP</span>
								on scan
							</div>
						</div>
						<div class="cd-scrl cd-pad">
							<div
								...
								@click="
									fireConfetti();
									earnXP('scan_card', { totalScans: (xp.totalScans || 0) + 1 });
								"
							>
							...
							<label class="cd-lbl">Title</label>
							<input v-model="addForm.title" class="cd-inp" placeholder="VP Product" />
							<label class="cd-lbl">Company</label>
							<input v-model="addForm.company" class="cd-inp" placeholder="Acme Corp" />
							<label class="cd-lbl">Email</label>
							<input v-model="addForm.email" type="email" class="cd-inp" placeholder="jane@acme.com" />
							...
							<label class="cd-lbl">Where We Met</label>
							<input v-model="addForm.metAt" class="cd-inp" placeholder="SaaS Summit NYC" />
					...
					<Transition name="cd-toast">
						<div v-if="toast" class="cd-toast">
							...
							<span style="font-family: var(--cd-bebas); font-size: 20px; color: var(--cd-accent); letter-spacing: 1px">
								{{ toast.xp }}
							</span>
		...
		<!-- ══ RIGHT PANEL ══════════════════════════════════════════════════════════ -->
		<div class="cd-panel overflow-scroll" v-if="showDetails">
			<div class="cd-panel-logo">
				CARD
				<span style="color: var(--cd-accent)">DESK</span>
			</div>
			<div style="font-size: 13px; color: var(--cd-dim); font-weight: 600; margin-bottom: 22px">
				Interactive Prototype · 6 Screens · Data persists in localStorage
			</div>
			...
			<div class="cd-guide-grid">
				<div v-for="s in PANEL_SCREENS" :key="s.name" class="cd-guide-card">
					<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px">
						...
						<div>
							<div style="font-family: var(--cd-cond); font-size: 15px; font-weight: 800; letter-spacing: 0.3px">
								{{ s.name }}
							</div>
							<div
								style="font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px"
								:style="'color:' + s.tc"
							>
								{{ s.tag }}
							</div>
			...
			<div class="cd-xp-table">
				<div v-for="row in PANEL_XP" :key="row.action" class="cd-xp-row">
					...
					<span style="font-family: var(--cd-mono); font-size: 12px; color: var(--cd-accent); font-weight: 700">
						{{ row.pts }}
					</span>
				</div>
			</div>
...
.cd-phone-col {
	...
	padding: 16px;
	background: radial-gradient(ellipse 70% 50% at 50% 0%, rgba(0, 255, 135, 0.05) 0%, transparent 60%), var(--cd-bg);
}
.cd-phone {
	width: 354px;
	height: 742px;
...
.cd-abtn.p:hover {
	background: rgba(184, 125, 255, 0.18);
}
.cd-abtn.ice {
...
.cd-vc.tip-vc {
	background: linear-gradient(135deg, #0e0720, #080414);
...
.cd-mcard.tg:hover .cd-mc-lbl {
	color: var(--cd-orange);
}
.cd-mcard.pk.sel .cd-mc-lbl,
.cd-mcard.pk:hover .cd-mc-lbl {
	color: var(--cd-accent);
}
.cd-mc-sub {
	font-size: 11px;
...
/* ── Cold ────────────────────────────────────────────────────────────────── */
.cd-cold-banner {
	background: linear-gradient(135deg, rgba(168, 216, 234, 0.07), rgba(77, 166, 255, 0.04));
...
/* ── Dashboard ───────────────────────────────────────────────────────────── */
.cd-hero {
	background: linear-gradient(135deg, #0b1f12 0%, #090f1e 60%, #0e091e 100%);
...
.cd-hero::before {
	...
	position: absolute;
	inset: 0;
	background: radial-gradient(ellipse 80% 60% at 20% 0%, rgba(0, 255, 135, 0.1) 0%, transparent 60%);
	pointer-events: none;
}
.cd-xp-track {
	height: 6px;
...
.cd-xp-fill {
	height: 100%;
	background: linear-gradient(90deg, var(--cd-ac2), var(--cd-accent));
	border-radius: 99px;
...
.cd-sdot.done {
	background: var(--cd-fire);
	box-shadow: 0 0 6px var(--cd-fire);
}
.cd-sdot.today {
	background: var(--cd-fire);
...
@keyframes cd-pf {
	...
	50% {
		box-shadow:
			0 0 16px var(--cd-fire),
			0 0 28px rgba(255, 69, 0, 0.4);
	}
}
.cd-mission {
	background: var(--cd-bg2);
...
.cd-badge.ul {
	...
	background: linear-gradient(135deg, rgba(255, 208, 0, 0.06), rgba(255, 208, 0, 0.02));
}
...
/* ── Contact detail ──────────────────────────────────────────────────────── */
.cd-det-hero {
	background: linear-gradient(180deg, var(--cd-bg3) 0%, var(--cd-bg) 100%);
	border: 1px solid var(--cd-bdr);
	border-radius: 15px;
	padding: 14px;
	margin-bottom: 11px;
...
.cd-fu-banner.overdue {
	background: linear-gradient(135deg, rgba(255, 107, 53, 0.1), rgba(255, 69, 0, 0.06));
	border: 1px solid rgba(255, 107, 53, 0.3);
}
.cd-fu-banner.due {
	background: linear-gradient(135deg, rgba(255, 224, 51, 0.08), rgba(255, 200, 0, 0.04));
...
.cd-scan-zone:hover {
	...
	background: linear-gradient(135deg, #061a10, #070f0c);
}
...
.cd-warmer-card {
	background: linear-gradient(135deg, rgba(168, 216, 234, 0.06), rgba(77, 166, 255, 0.03));
	border: 1px solid rgba(168, 216, 234, 0.15);
	border-radius: 11px;
	padding: 11px 13px;
	display: flex;

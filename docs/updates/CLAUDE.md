...
const directus = useDirectus() // typed client from server/utils/directus.ts
```
...
|------------|------------|
| `organizations` | `plan`, `ai_token_balance`, `ai_token_limit_monthly`, `ai_tokens_used_this_period`, `ai_billing_period_start` |
...
| `phone_settings` | `twilio_phone_number`, `line_name`, `organization` (add this field) |

## Current plan tiers (after pricing update)

```typescript
// These are the TARGET values — update stripe.ts to match
...
```
free:    basic PM, tickets, invoicing only
solo:    + CRM, social scheduling, email marketing, AI (100K tokens)
studio:  + all features, team features, AI (400K tokens)
agency:  + white-label, advanced AI, unlimited scans
```

## Video meeting architecture

Currently uses Twilio Video (room_sid on video_meetings collection).
Target: migrate to Daily.co. The collection shape stays the same — `room_name` becomes the
Daily room name, `room_sid` can store the Daily room ID or be left null.
Daily.co API: `https://api.daily.co/v1/` with Bearer token from `DAILY_API_KEY` env var.

## Twilio architecture (phones, SMS)

Currently: single Twilio account, phone numbers stored in `phone_settings.twilio_phone_number`.
Target: Twilio sub-accounts per organization. Add `twilio_subaccount_sid` and
`twilio_subaccount_token` fields to `organizations` collection. The master account provisions
sub-accounts; all calls/SMS for an org route through its sub-account.
...
STRIPE_PRICE_SOLO / STRIPE_PRICE_STUDIO / STRIPE_PRICE_AGENCY
STRIPE_PRICE_TOKENS_500K / STRIPE_PRICE_TOKENS_2M / STRIPE_PRICE_TOKENS_10M
DAILY_API_KEY
TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN (master account)
```
